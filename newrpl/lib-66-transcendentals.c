...
#define COMMAND_LIST \
    ...
    ECMD(SQRT,"√",MKTOKENINFO(1,TITYPE_PREFIXOP,1,4)), \
    ECMD(PINUM,"π0",MKTOKENINFO(2,TITYPE_PREFIXOP,0,3)), \
    CMD(PSIN,MKTOKENINFO(4,TITYPE_FUNCTION,1,2)), \
    CMD(PCOS,MKTOKENINFO(4,TITYPE_FUNCTION,1,2)), \
    CMD(PTAN,MKTOKENINFO(4,TITYPE_FUNCTION,1,2)), \
    CMD(PASIN,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    CMD(PACOS,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    CMD(PATAN,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    CMD(PATAN2,MKTOKENINFO(6,TITYPE_FUNCTION,2,2)), \
    CMD(PLN,MKTOKENINFO(3,TITYPE_FUNCTION,1,2)), \
    CMD(PEXP,MKTOKENINFO(4,TITYPE_FUNCTION,1,2)), \
    CMD(PSINH,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    CMD(PCOSH,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    CMD(PTANH,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    CMD(PASINH,MKTOKENINFO(6,TITYPE_FUNCTION,1,2)), \
    CMD(PACOSH,MKTOKENINFO(6,TITYPE_FUNCTION,1,2)), \
    CMD(PATANH,MKTOKENINFO(6,TITYPE_FUNCTION,2,2)), \
    CMD(PLOG,MKTOKENINFO(4,TITYPE_FUNCTION,1,2)), \
    CMD(PALOG,MKTOKENINFO(5,TITYPE_FUNCTION,1,2)), \
    ECMD(PSQRT,"P√",MKTOKENINFO(2,TITYPE_PREFIXOP,1,4))
...
void LIB_HANDLER()
{
...

    {
        ...
        case CPLX_INF:
        {
            ...
            // TANH(+/-Inf)=+/- 1
            rplDropData(1);
            if(re.flags&F_NEGATIVE) rplNewBINTPush(-1,DECBINT);
    ...
    case ATANH:
    {
...

        {
            ...
            if(cmpReal(&im,&pi2)>=0) RReg[0].flags|=F_NEGATIVE;
    ...
    // *****************************************************************************************************
    // **************  SAME COMMANDS BUT WITH LIGHT TRANSCENDENTALS VERSION ********************************
   ...
   // STANDARIZED OPCODES:
    ...
    {
        if((MENUNUMBER(MenuCodeArg)<=3)) ObjectPTR=ROMPTR_TABLE[MENUNUMBER(MenuCodeArg)+2];
        else ObjectPTR=(WORDPTR)empty_list;
