...
//@TITLE=Time, Alarms and System Commands
...
#define COMMAND_LIST \
    ECMD(SETDATE,"→DATE",MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    ECMD(DATEADD,"DATE+",MK_TOKEN_INFO(5,TITYPE_FUNCTION,2,2)), \
    ECMD(SETTIME,"→TIME",MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    ECMD(TOHMS,"→HMS",MK_TOKEN_INFO(4,TITYPE_NOTALLOWED,1,2)), \
    ECMD(FROMHMS,"HMS→",MK_TOKEN_INFO(4,TITYPE_NOTALLOWED,1,2)), \
    ECMD(HMSADD,"HMS+",MK_TOKEN_INFO(4,TITYPE_FUNCTION,2,2)), \
    ECMD(HMSSUB,"HMS-",MK_TOKEN_INFO(4,TITYPE_FUNCTION,2,2)), \
    CMD(TICKS,MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    CMD(TEVAL,MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    CMD(DATE,MK_TOKEN_INFO(4,TITYPE_FUNCTION,0,2)), \
    CMD(DDAYS,MK_TOKEN_INFO(5,TITYPE_FUNCTION,2,2)), \
    CMD(TIME,MK_TOKEN_INFO(4,TITYPE_FUNCTION,0,2)), \
    CMD(TSTR,MK_TOKEN_INFO(4,TITYPE_NOTALLOWED,2,2)), \
    CMD(ACK,MK_TOKEN_INFO(3,TITYPE_NOTALLOWED,0,2)), \
    CMD(ACKALL,MK_TOKEN_INFO(6,TITYPE_NOTALLOWED,0,2)), \
    CMD(RCLALARM,MK_TOKEN_INFO(8,TITYPE_NOTALLOWED,1,2)), \
    CMD(STOALARM,MK_TOKEN_INFO(8,TITYPE_NOTALLOWED,1,2)), \
    CMD(DELALARM,MK_TOKEN_INFO(8,TITYPE_NOTALLOWED,1,2)), \
    CMD(FINDALARM,MK_TOKEN_INFO(9,TITYPE_NOTALLOWED,1,2)), \
    CMD(VERSION,MK_TOKEN_INFO(7,TITYPE_NOTALLOWED,0,2)), \
    CMD(MEM,MK_TOKEN_INFO(3,TITYPE_NOTALLOWED,0,2)), \
    CMD(BYTES,MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    CMD(PEEK,MK_TOKEN_INFO(4,TITYPE_NOTALLOWED,1,2)), \
    CMD(POKE,MK_TOKEN_INFO(4,TITYPE_NOTALLOWED,1,2)), \
    CMD(NEWOB,MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    CMD(GARBAGE,MK_TOKEN_INFO(7,TITYPE_NOTALLOWED,1,2)), \
    CMD(MEMCHECK,MK_TOKEN_INFO(8,TITYPE_NOTALLOWED,1,2)), \
    CMD(MEMFIX,MK_TOKEN_INFO(6,TITYPE_NOTALLOWED,1,2)), \
    ECMD(USBFWUPDATE,"FWUPD",MK_TOKEN_INFO(5,TITYPE_NOTALLOWED,1,2)), \
    CMD(DOALARM,MK_TOKEN_INFO(7,TITYPE_NOTALLOWED,0,2)), \
    CMD(ALRM,MK_TOKEN_INFO(4,TITYPE_NOTALLOWED,1,2)), \
    CMD(OFF,MK_TOKEN_INFO(3,TITYPE_NOTALLOWED,1,2))
...
INCLUDE_ROMOBJECT(lib65_menu_3_others);
...
{
    if(!rplReadRealAsDateNoCk(date, dt))
        return 0;
...
int32_t rplReadRealAsDateNoCk(REAL * date, struct date *dt)
{
    ...
    if(!inint32_tRange(&r_date) || isNANorinfiniteReal(&r_date)
            || (r_date.flags & F_NEGATIVE))
        return 0;

    year = getint32_tReal(&r_date);
    month = year / 1000000;
    year -= month * 1000000;
    day = year / 10000;
    year -= day * 10000;
    ...
    if(day > DATE_MAXDAY)
        return 0;
    if(month > DATE_MAXMON)
        return 0;
    if(year > DATE_MAXYEAR)
        return 0;
...
int32_t rplReadDateAsReal(struct date dt, REAL * date)
{
    ...
    newRealFromint32_t(date, b_date, -6);
...
int32_t rplIsValidDate(struct date dt)
{
    ...
    if(dt.mon < 1 || dt.mon > 12)
        return 0;
    if(dt.mday < 1 || dt.mday > rplGetMonthDays(dt.mon, dt.year))
        return 0;

    if(dt.year == 1582) {
        if(dt.mon < 10)
            return 0;
        if((dt.mon == 10) && (dt.mday < 15))
            return 0;
    }
...
int32_t rplReadRealAsTime(REAL * time, struct time *tm)
{
    ...
    if(!inint32_tRange(&r_time) || isNANorinfiniteReal(&r_time)
            || (r_time.flags & F_NEGATIVE))
        return 0;

    sec = getint32_tReal(&r_time);
    hour = sec / 10000;
    sec -= hour * 10000;
    min = sec / 100;
    sec -= min * 100;
...
int32_t rplReadTimeAsReal(struct time tm, REAL * time)
{
    ...
    newRealFromint32_t(time, b_time, -4);
...
{
    struct date dt;
    int32_t mon, year, ddays;

    // ADD DAY NUMBER OF OCTOBER 15, 1582, SINCE JANUARY 0, 0
    days += 578040;

    year = (int32_t) ((10000 * (int64_t) days + 14780) / 3652425);
...
static int32_t ReadSysAlarm(word_p list, struct alarm *alrm)
{
    ...
    if(!ISint32_t(*obj))
        return 0;
    alrm->time = rplReadint32_t(obj);

    obj = rplGetListElement(list, 2);
    if(!ISint32_t(*obj))
        return 0;
    alrm->rpt = (uint32_t) rplReadint32_t(obj);

    obj = rplGetListElement(list, 3);
    if(!ISint32_t(*obj))
        return 0;
    alrm->flags = (BYTE) rplReadint32_t(obj);
...
static int32_t AddSysAlarm(struct alarm *new_alrm)
{
    ...
    first_due_id = rplReadint32_t(rplPeekData(nalarms + 2));
    past_due_id = rplReadint32_t(rplPeekData(nalarms + 1));
    if(id <= first_due_id)
        first_due_id++;
    if(id <= past_due_id)
        past_due_id++;
...
static int32_t GetSysAlarm(int32_t id, struct alarm *alrm)
{
    int32_t nalarms;
...
static int32_t FindNextAlarm(int64_t time, struct alarm *alrm)
{
    int32_t id = 0;
...
// RETURN 0 ON ERROR
// CALLER MUST CALL ScanAlarms
static int32_t UpdateSysAlarm(int32_t id, struct alarm *alrm)
{
    alrm_obj = alrm->obj;
    if(!DelSysAlarm(id)) {
        alrm->obj = alrm_obj;
...
static int32_t GetFirstAlarmId(int32_t type)
{
    ...
    return (int32_t) rplReadint32_t(bint);
}
...
static int32_t GetFirstAlarm(int32_t type, struct alarm *alrm)
{
    int32_t id = GetFirstAlarmId(type);
...
// CALLER MUST CALL ScanAlarms
static int32_t RescheduleAlarm(int32_t id, struct alarm *alrm)
{
    struct alarm stored;
...
static int32_t AckSequence(struct alarm *alrm)
{
    int32_t i, ack;
    char *msg_start, *msg_end;
...
int32_t rplReadAlarm(word_p obj, struct alarm *alrm)
{
    ...
    if(alarm_rpt == NULL) {
        ...
        b_alarm_rpt = rplReadNumberAsInt64(alarm_rpt);
        if((b_alarm_rpt < 0) || (b_alarm_rpt >= (1LL << 32))) {
            rplError(ERR_INVALIDRPT);
            return 0;
        }
        alrm->rpt = (uint32_t) b_alarm_rpt;
    }
...
void LIB_HANDLER()
{
    ...
    case RCLALARM:
    {
        ...
        id = rplReadNumberAsInt64(arg_id);
    ...
    case PEEK:
    {
        ...
        int64_t addr = rplReadint32_t(rplPeekData(1));
    ...
    case POKE:
    {
        ...
        int64_t addr = rplReadint32_t(rplPeekData(2));
        int64_t value = rplReadint32_t(rplPeekData(1));
    ...
    {
        if(MENU_NUMBER(MenuCodeArg) > 3) {
            RetNum = ERR_NOTMINE;
            return;
        }
        ObjectPTR = (word_p) ROMPTR_TABLE[MENU_NUMBER(MenuCodeArg) + 2];
        RetNum = OK_CONTINUE;
        return;
    }
