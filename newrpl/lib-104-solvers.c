...
#define COMMAND_LIST \
    CMD(NUMINT,MK_TOKEN_INFO(6,TITYPE_FUNCTION,4,2)), \
    CMD(ROOT,MK_TOKEN_INFO(4,TITYPE_FUNCTION,3,2)), \
    CMD(MSOLVE,MK_TOKEN_INFO(6,TITYPE_FUNCTION,5,2)), \
    CMD(BISECT,MK_TOKEN_INFO(6,TITYPE_FUNCTION,3,2))
...
#define ARG_ERROR *(dstkptr-1)
    ...
    case MSOLVE:
    {
            ...
            while(tries < maxtries) {
                ...
                for(i = 1; i <= nvars; ++i) {
                    ...
                    default:
                        ...
                        if(i <= j)
                            rplPushData(rplGetListElement(*listmin, i));
                        else
                            rplPushData(rplGetListElement(*listmax, i));
                        if(Exceptions) {
                            DSTop = stksave;
                            return;
                        }
                    ...
                    case 1:
                        ...
                        rplReadNumberAsReal(rplGetListElement(*listmin, i),
                                &x1);
                        rplReadNumberAsReal(rplGetListElement(*listmax, i),
                                &x2);
        ...
        char Buffer0[5000],*chrptr=Buffer0;
        int lf,lk;
        for(lf=nvars+1;lf>=1;--lf) {
        chrptr=Buffer0;
        sprintf(chrptr,"%02d: [ ",lf);
        chrptr+=6;
        for(lk=1;lk<=nvars+1;++lk) {
        rplReadNumberAsReal(rplGetListElement(rplPeekData(lf),lk),&num);
        chrptr=formatReal(&num,chrptr,FMT_DIGITS(6),MAKELOCALE('.',' ',' ',','));
        *chrptr++=' ';
        }
        ...
        *chrptr=0;
        ...
        for(i = 1; i <= nvars; ++i) {
            for(j = nvars; j >= i; --j) {
                rplReadNumberAsReal(rplGetListElement(rplPeekData(j),
                            nvars + 1), &x1);
                rplReadNumberAsReal(rplGetListElement(rplPeekData(j + 1),
                            nvars + 1), &x2);
                if(ltReal(&x1, &x2)) {
                    // SWAP
        ...
        *chrptr=0;
        ...
        do {
            ...
            rplReadNumberAsReal(rplGetListElement(pointarray[0], nvars + 1),
                    &x1);
            rplReadNumberAsReal(rplGetListElement(pointarray[nvars], nvars + 1),
                    &x2);
            ...
            if(lteReal(&RReg[0], &tolerance)
                    && (intdigitsReal(&x1) > intdigitsReal(&tolerance))) {
                if(noprogress > 3)
                {
#ifdef DEBUG_MEAD_NELDER
                    printf("No progress between worst and best points. Exiting the loop.\n");
                    fflush(stdout);
#endif
                    break;      // WITHIN TOLERANCE, EXIT MAIN LOOP
                }
                ++noprogress;
            }
            else
            ...
            if(nvars >= 2) {
                ...
                // SAVE IN RReg[9] THE AVERAGE DISTANCE FROM X0 AND CENTROID
                rplZeroToRReg(9);
                int outoftolerance=0;

                for(j = 1; j <= nvars; ++j) {
                    rplReadNumberAsReal(rplGetListElement(rplPeekData(nvars +
                                    2), j), &x1);
                    rplReadNumberAsReal(rplGetListElement(rplPeekData(1), j),
                            &x2);
                    subReal(&RReg[0], &x1, &x2);
                    RReg[0].flags &= ~F_NEGATIVE;

                    addReal(&RReg[9],&RReg[9],&RReg[0]);        // ACCUMULATE IN RReg[9]

                    // NOW COMPUTE DIFFERENCE WITH WORST POINT
                    rplReadNumberAsReal(rplGetListElement(rplPeekData(2), j), &x1);

                    subReal(&RReg[0], &x1, &x2);
                    RReg[0].flags &= ~F_NEGATIVE;

                    if(gtReal(&RReg[0], &tolerance))
                ...
                {
#ifdef DEBUG_MEAD_NELDER
                    printf("Worst point and centroid close to tolerance. Exitting...\n");
                    fflush(stdout);
#endif
                    break;      // ALL COMPONENTS WITHIN TOLERANCE, BREAK OUT OF THE MAIN LOOP
                }
            }
            else {
                ...
                copyReal(&RReg[9],&RReg[0]);    // RReg[9]= AVERAGE COMPONENT DISTANCE FROM X0 TO C, TO BE USED DURING GRADIENT STEP

                if(lteReal(&RReg[0], &tolerance))
                {
#ifdef DEBUG_MEAD_NELDER
                    printf("Best point and centroid close to tolerance. Exitting...\n");
                    fflush(stdout);
#endif
                    break;      // WITHIN TOLERANCE, EXIT MAIN LOOP
                }
            ...
            {
            REAL num;
            printf("Average distance from centroid to X0: ");
            char Buffer0[5000],*chrptr=Buffer0;
            chrptr=formatReal(&RReg[9],chrptr,FMT_DIGITS(6),MAKELOCALE('.',' ',' ',','));
            *chrptr=0;
            printf("%s\n",Buffer0);
            fflush(stdout);
            }
            ...
            chrptr+=6;
            for(lk=1;lk<=nvars+1;++lk) {
            rplReadNumberAsReal(rplPeekData(nvars+2-lk),&num);
            chrptr=formatReal(&num,chrptr,FMT_DIGITS(6),MAKELOCALE('.',' ',' ',','));
            *chrptr++=' ';
            }
            ...
            *chrptr=0;
            ...
            if(!iszeroReal(&RReg[8])) {
                ...
                // AND APPLY A UNIT LENGTH = AVERAGE DISTANCE FROM X0 TO THE CENTROID

                // TEST ONLY:
                swapReal(&RReg[9],&RReg[0]);
                addReal(&RReg[9],&RReg[0],&RReg[0]);    // TWICE AVERAGE DISTANCE


                divReal(&RReg[2], &RReg[9], &RReg[8]);
            ...
            *chrptr=0;
            ...
            if(ltReal(&x1, &fx)) {
                ...
                swapReal(&x1, &fx);
#ifdef DEBUG_MEAD_NELDER
                printf("Chose Gradient point G'\n");
#endif
            }
            else
            {
                rplDropData(nvars + 1); // OTHERWISE KEEP THE ORIGINAL POINT P
#ifdef DEBUG_MEAD_NELDER
                printf("Gradient didn't improve on P, keeping P.\n");
#endif
            }
            ...
            {
            REAL num;
            printf("P didn't improve - Shrink Simplex\nUPDATED point set:\n");
            char Buffer0[5000],*chrptr=Buffer0;
            int lf,lk;
            for(lf=nvars+1;lf>=1;--lf) {
            chrptr=Buffer0;
