...
#define COMMAND_LIST \
    ECMD(PICONST,"π",MKTOKENINFO(1,TITYPE_CONSTANTIDENT,0,1)), \
    ECMD(ECONST,"е",MKTOKENINFO(1,TITYPE_CONSTANTIDENT,0,1)), \
    ECMD(ICONST,"і",MKTOKENINFO(1,TITYPE_CONSTANTIDENT,0,1)), \
    ECMD(JCONST,"ј",MKTOKENINFO(1,TITYPE_CONSTANTIDENT,0,1)), \
    ECMD(KCONST,"к",MKTOKENINFO(1,TITYPE_CONSTANTIDENT,0,1))
...
void LIB_HANDLER()
{
    ...
    case OPCODE_COMPILE:
        ...
        if(RetNum==OK_CONTINUE) {
            // ENCAPSUALTE THE OPCODE INSIDE AN OBJECT
            rplCompileAppend(MAKESINT(OPCODE(*(CompileEnd-1))));
