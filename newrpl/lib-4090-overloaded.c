...
#define OP_LIST \
    OP(INV,MK_TOKEN_INFO(3,TITYPE_FUNCTION,1,2)), \
    OP(NEG,MK_TOKEN_INFO(3,TITYPE_FUNCTION,1,4)), \
    OP(EVAL1,MK_TOKEN_INFO(5,TITYPE_FUNCTION,1,2)), \
    OP(EVAL,MK_TOKEN_INFO(4,TITYPE_FUNCTION,1,2)), \
    OP(FUNCEVAL,MK_TOKEN_INFO(8,TITYPE_CUSTOMFUNC,15,2)), \
    OP(XEQ,MK_TOKEN_INFO(3,TITYPE_FUNCTION,1,2)), \
    OP(NUM,MK_TOKEN_INFO(4,TITYPE_FUNCTION,1,2)), \
    OP(ABS,MK_TOKEN_INFO(3,TITYPE_FUNCTION,1,2)), \
    OP(ISTRUE,MK_TOKEN_INFO(6,TITYPE_FUNCTION,1,2)), \
    OP(NOT,MK_TOKEN_INFO(3,TITYPE_FUNCTION,1,2)), \
    OP(EQ,MK_TOKEN_INFO(2,TITYPE_BINARYOP_LEFT,2,10)), \
    OP(NOTEQ,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,10)), \
    OP(LT,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,9)), \
    OP(GT,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,9)), \
    OP(LTE,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,9)), \
    OP(GTE,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,9)), \
    OP(SAME,MK_TOKEN_INFO(4,TITYPE_FUNCTION,2,2)), \
    OP(AND,MK_TOKEN_INFO(3,TITYPE_BINARYOP_LEFT,2,11)), \
    OP(OR,MK_TOKEN_INFO(2,TITYPE_BINARYOP_LEFT,2,13)), \
    OP(XOR,MK_TOKEN_INFO(3,TITYPE_BINARYOP_LEFT,2,12)), \
    OP(CMP,MK_TOKEN_INFO(3,TITYPE_FUNCTION,2,9)), \
    OP(ADD,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,8)), \
    OP(SUB,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,7)), \
    OP(MUL,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,6)), \
    OP(DIV,MK_TOKEN_INFO(1,TITYPE_BINARYOP_LEFT,2,5)), \
    OP(POW,MK_TOKEN_INFO(1,TITYPE_BINARYOP_RIGHT,2,3)), \
    OP(XROOT,MK_TOKEN_INFO(5,TITYPE_FUNCTION,2,2))
...
#define OP(a,b) CMD_OVR_##a = MK_OPCODE(LIBRARY_NUMBER,OVR_##a)
enum
{ OP_LIST, OP(UPLUS, xx), OP(UMINUS, xx) };
#undef OP
...
void rplCallOvrOperator(WORD op)
{
    ...
    if(han) {
        // EXECUTE THE OTHER LIBRARY DIRECTLY
        int32_t SavedOpcode = CurOpcode;
        CurOpcode = MK_OPCODE(LIBRARY_NUMBER, op);
        (*han) ();
        if(CurOpcode == MK_OPCODE(LIBRARY_NUMBER, op))
            CurOpcode = SavedOpcode;
    }
    else {
...
void LIB_HANDLER()
{
    ...
    if(OPCODE(CurOpcode) >= MIN_RESERVED_OPCODE) {
        ...
        case OPCODE_DECOMPILE:
            ...
            if(OPCODE(*DecompileObject) == OVR_UMINUS) {
                rplDecompAppendChar('-');
                RetNum = OK_CONTINUE;
                return;
            }
            if(OPCODE(*DecompileObject) == OVR_UPLUS) {
                rplDecompAppendChar('+');
                RetNum = OK_CONTINUE;
                return;
            }
        ...
        case OPCODE_GETINFO:
            ...
            if(OPCODE(*DecompileObject) == OVR_UMINUS) {
                RetNum = OK_TOKENINFO | MK_TOKEN_INFO(1, TITYPE_PREFIXOP, 1, 4);
                return;
            }
            if(OPCODE(*DecompileObject) == OVR_UPLUS) {
                RetNum = OK_TOKENINFO | MK_TOKEN_INFO(1, TITYPE_PREFIXOP, 1, 4);
        ...
        case OPCODE_AUTOCOMPNEXT:
        {
            ...
            do {
                if(LIBNUM(SuggestedOpcode) == LIBRARY_NUMBER) {
                    ...
                    if(k == LIB_NUMBEROFCMDS) {
                        RetNum = ERR_NOTMINE;
                        return;
                    }
                    SuggestedOpcode = MK_OPCODE(LIBRARY_NUMBER, k);
                }
                ...
                if(RetNum == OK_CONTINUE) {
                    ...
                    SuggestedOpcode =
                            MK_OPCODE(LIBRARY_NUMBER, LIB_OPCODES[idx]);
                }
                else
                    idx = 0;
            }
