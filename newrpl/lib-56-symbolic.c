...
void LIB_HANDLER()
{
    ...
    if((OPCODE(CurOpcode) >= MIN_OVERLOAD_OPCODE)
            ...
            if(Opcode) {
                ...
                if(OPCODE(Opcode) == OVR_FUNCEVAL) {
                    ...
                    if((TI_TYPE(tokeninfo) == TITYPE_CASFUNCTION)
                        ...
                        rplPushRet(IPtr);
                        if((rplPeekRet(1) < symbeval1_seco)
            ...
            // ADDITION IS A SPECIAL CASE, NEEDS TO KEEP ARGUMENTS FLAT
        {
            ...
            if((!rplIsAllowedInSymb(rplPeekData(2)))
                    || (!rplIsAllowedInSymb(rplPeekData(1)))) {
            ...
            if((ISSYMBOLIC(*arg1)||ISIDENT(*arg1)||ISCONSTANT(*arg1)) && rplSymbMainOperator(arg1) == (CMD_OVR_ADD)) {
                ...
                while(ScratchPointer1 != ScratchPointer2) {
                    if(ISSYMBOLIC(*ScratchPointer1)
                            && rplSymbMainOperator(ScratchPointer1) ==
                            (CMD_OVR_ADD)) {
        ...
        case OVR_SAME:
        {
            ...
            while(same && (ScratchPointer1 < ScratchPointer3)
                ...
                if(ISPROLOG(*ScratchPointer1) || ISint32_t(*ScratchPointer1)
                    ...
                    if(*ScratchPointer1 != *ScratchPointer2)
                        same = 0;
                }
            ...
            rplDropData(2);
            if(same && (ScratchPointer1 == ScratchPointer3)
                    && (ScratchPointer2 == ScratchPointer4))
                rplPushTrue();
            else
                rplPushFalse();
    ...
    case SYMBEVAL1ERR:
        ...
        if((ExceptionPointer < *rplGetLAMn(4))
                || (ExceptionPointer >= rplSkipOb(*rplGetLAMn(4))))
    ...
    case AUTOSIMPLIFY:
    {
        ...
        if(!ISSYMBOLIC(*rplPeekData(1)))
            return;     // LEAVE IT ON THE STACK, NOT A SYMBOLIC
    ...
    case SYMBNUMERR:
        ...
        while((IPtr >= symbnum_seco) && (IPtr < symbnum_seco + 5)) {
            if(RSTop <= RStk)
                break;  // NOTHING ELSE ON THE STACK!?
            IPtr = rplPopRet();
        }
...
#define ONE_REG(reg) { reg.len=1; reg.data[0]=1; reg.exp=0; reg.flags=0; }
        ...
        addReal(&t2, &t2, &d);
        if(s < 0)
            t2.flags ^= F_NEGATIVE;     //   t2:=(n*t1+d)*s;
...

    {
        ...
        if(ISSYMBOLIC(*argstart) || ISIDENT(*argstart) || ISCONSTANT(*argstart)
                || ISSYMBOLIC(*argend) || ISIDENT(*argend)
                || ISCONSTANT(*argend) || ISSYMBOLIC(*argstep)
                || ISIDENT(*argstep) || ISCONSTANT(*argstep)
                ) {
        ...
        if(RReg[0].flags & F_NEGATIVE) {
            if(iszeroReal(&step) || !(step.flags & F_NEGATIVE))
                addReal(&RReg[0], &RReg[0], &RReg[0]);  // STEP=(END-REAL)*2 SO ONLY ONE POINT WILL BE RETURNED
            else
                copyReal(&RReg[0], &step);
        }
        else {
            if(iszeroReal(&step) || (step.flags & F_NEGATIVE))
                addReal(&RReg[0], &RReg[0], &RReg[0]);  // STEP=(END-REAL)*2 SO ONLY ONE POINT WILL BE RETURNED
