        ...
        if(ISIDENT(*direntry[0]) && (OBJSIZE(*direntry[0]) == 1)) {
            ...
            if(direntry[0][1] == libid) {
                ...
                if(libcmd == 0)
                    ...
                    nobj = rplSkipOb(direntry[1] + OPCODE(direntry[1][libcmd +
                                    3]));
...
void LIB_HANDLER()
{
                    ...
                    if(ISLIST(*item) && (rplListLength(item) == 4)) {
                        ...
                        if(ISIDENT(*var)) {
                            ...
                            if(prog) {
                                ...
                                datasize +=
                                        rplObjSize(rplPeekData(2)) +
                                        rplObjSize(rplPeekData(1)) + 1;
                                var = rplSkipOb(var);
                                if(ISNUMBER(*var) && ISNUMBER(*rplSkipOb(var))
                                        && ISSTRING(*rplSkipOb(rplSkipOb(var))))
                                {
    ...
    case LIBSTO:
    {
        ...
        var = rplFindGlobalInDir(rplPeekData(1), rplFindDirbyHandle(dirhandle),
                0);

        if(!var)
            rplCreateGlobalInDir(rplPeekData(1), rplPeekData(2),
                    rplFindDirbyHandle(dirhandle));
        else
            var[1] = rplPeekData(2);
        if(Exceptions)
            return;
                ...
                do {
                    ...
                    if(ISIDENT(*direntry[0]) && (OBJSIZE(*direntry[0]) == 1)) {
                        ...
                        if(prevlibid) {
                            ...
                            if(direntry[0][1] != prevlibid)
                                continue;
                            // FOUND IT, previdx HAS THE START INDEX
                            prevlibid = 0;
                            if(previdx <= 4)
                                continue;       // THE PREVIOUS COMMAND WAS THE FIRST COMMAND, SKIP TO THE NEXT LIBRARY
                        }
                        else
                            previdx = nentries;
