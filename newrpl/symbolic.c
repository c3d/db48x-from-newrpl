...
void rplSymbReplaceMatchHere(WORDPTR *rule,BINT startleftarg)
{
    ...
    right=DSTop-4;
    left=right-1;
      if(!ISPROLOG(**right) && !ISBINT(**right)) { rightnargs=rplReadBINT(*(right-1)); --left; }
    else rightnargs=0;
    left-=rightnargs;
    ...
    // GET EXPRESSION TO REPLACE WITH
        ...
        if(leftnargs) {
            ...
            leftnargs-=rightnargs-1;
            left-=rightnargs-1;
            left[-1]=rplNewSINT(leftnargs,DECBINT);
            }
            else {
                // SINCE ALL ARGUMENTS ARE REPLACED, ALSO REPLACE THE OPERATOR
                *left=newsymb;
                rplRemoveAtData(DSTop-&FINDARGUMENT(left,leftnargs,1+leftnargs),leftnargs+1);
                left-=leftnargs+1;
                leftidx=leftnargs=0;
        ...
        while(leftidx>=0) {
            ...
            rplExpandStack(leftnargs);
            if(Exceptions) { DSTop=stksave; return; }
            for(k=1;k<=leftnargs;++k) *DSTop++=FINDARGUMENT(left,leftnargs,k);
            rplSymbApplyOperator(**left,leftnargs);
            }
            else {
             rplPushData(*left);
            }
        ...
        DSBase=left- ( (leftnargs)? (1+leftnargs):0);
        right=DSBase-4;
...
WORDPTR *left,*right,*marker,*baselevel;
...
do {
                ...
                if(lamname) {
                        ...
                        // TAKE ALL ARGUMENTS OF THE OPERATION
                    {
                        ...
                        if((**pleft==CMD_OVR_ADD)||(**pleft==CMD_OVR_MUL))  // IT'S A COMMUTATIVE/ASSOCIATIVE OPERATOR
                        {
                            ...
                            if(prightidx!=prightnargs) { // THE SPECIAL IDENT IS NOT THE LAST ARGUMENT

                                WORDPTR tmp=FINDARGUMENT(pright,prightnargs,prightidx);
                                ...
                                for(k=prightidx;k<prightnargs;++k)  FINDARGUMENT(pright,prightnargs,k)=FINDARGUMENT(pright,prightnargs,k+1);
                                FINDARGUMENT(pright,prightnargs,k)=tmp;

                                // THEN REPLACE THE right PART ON THE CURRENT EXPRESSION AND REDO THIS OP_MATCH
                                DSTop=left+1;

                                // PUSH THE RIGHT
                                rplSymbExplodeOneLevel2(FINDARGUMENT(pright,prightnargs,prightidx));
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                                right=DSTop;

                                // LEFTARG AND RIGHTARG
                                rplNewSINTPush(leftidx,DECBINT);
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                                rplNewSINTPush(rightidx,DECBINT);
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                                rplPushData((WORDPTR)zero_bint);
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                            ...
                            for(k=pleftidx;k<=pleftnargs;++k) {
                                rplPushData(FINDARGUMENT(pleft,pleftnargs,k));
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                            }
                            if(pleftnargs>pleftidx) {
                                rplSymbApplyOperator(**pleft,pleftnargs-pleftidx+1);
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                            }
                            ...
                            if(pleftnargs>pleftidx) {
                                rplRemoveAtData(DSTop-&FINDARGUMENT(pleft,pleftnargs,pleftnargs),pleftnargs-pleftidx);
                                pleft-=pleftnargs-pleftidx;
                                if(baselevel>pleft) baselevel-=pleftnargs-pleftidx;
                                pleft[-1]=rplNewSINT(pleftidx,DECBINT);
                            }
                        ...
                        // m = Match smallest expression with any variables or constants, assuming commutative addition but non-commutative multiplication (use this with matrix expressions)
                    {
                            ...
                            WORDPTR *pleft,*pright;
                            BINT pleftnargs,prightnargs,pleftidx,prightidx,pleftrot,plrotbase;
                        ...
                        if(plrotbase && pleftrot && (**pleft==CMD_OVR_MUL)) {
                        ...
                        // TAKE ALL ARGUMENTS OF THE OPERATION
                    {
                        ...
                        if(**pleft==CMD_OVR_ADD)  // IT'S A COMMUTATIVE/ASSOCIATIVE OPERATOR
                        {
                        ...
                        else if(**pleft==CMD_OVR_MUL) {
                            // IT'S NON-COMMUTATIVE BUT ASSOCIATIVE OPERATOR, TAKE ALL ARGUMENTS LESS WHAT'S LEFT ON THE RIGHT SIDE
                            BINT k;

                            BINT otherright=prightnargs-prightidx;  // OTHER ARGUMENTS ON THE RIGHT OPERATOR AFTER THIS ONE
                            BINT available=pleftnargs-pleftidx+1-otherright; // NUMBER OF ARGUMENTS AVAILABLE FOR THIS EXPRESSION

                            if(available>1) {
                            // CREATE A SYMBOLIC WITH THE SAME OPERATOR AND ALL REMAINING ARGUMENTS
                            // AND ASSIGN IT TO THIS VARIABLE
                            for(k=pleftidx;k<=pleftnargs-otherright;++k) {
                                rplPushData(FINDARGUMENT(pleft,pleftnargs,k));
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                            }

                                rplSymbApplyOperator(**pleft,available);
                                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                            ...
                            // FINALLY, REMOVE ALL EXTRA ARGUMENTS FROM THE PARENT
                            rplRemoveAtData(DSTop-&FINDARGUMENT(pleft,pleftnargs,pleftnargs-otherright),available-1);
                                pleft-=available-1;
                                if(baselevel>pleft) baselevel-=available-1;
                                pleft[-1]=rplNewSINT(pleftnargs-(available-1),DECBINT);
    ...
    case ARGMATCH:
    {
        ...
        if(leftidx<=leftnargs) {
            if(rightidx>rightnargs) {
                ...
                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                // PUSH THE RIGHT
                rplSymbExplodeOneLevel2(FINDARGUMENT(right,rightnargs,rightidx));
                if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                right=DSTop;
    ...
    case BACKTRACK:
    {
        ...
        // OTHERWISE BACKTRACK TO UPPER LEVEL
        // GET POINTERS
        right=DSTop-4;
        left=right-1;
          if(!ISPROLOG(**right) && !ISBINT(**right)) { rightnargs=rplReadBINT(*(right-1)); --left; }
        ...
        if(DSTop==baselevel) {
            ...
            if(matchstarted && DSTop>baselevel) {
                ...
                if(leftnargs && (leftidx>0) && lrotbase) {

                    if(leftrot<leftnargs-lrotbase) {
                    // COMMUTATIVE OPERATORS NEED TO ROT THE ARGUMENTS AND TRY AGAIN
                    WORDPTR tmp=FINDARGUMENT(left,leftnargs,lrotbase);
                    BINT k;
                    for(k=lrotbase;k<leftnargs;++k) FINDARGUMENT(left,leftnargs,k)=FINDARGUMENT(left,leftnargs,k+1);
                    FINDARGUMENT(left,leftnargs,leftnargs)=tmp;
                    matchtype=ARGMATCH;
                    ++leftrot;
                    right[3]=rplNewBINT((((BINT64)lrotbase)<<32)|leftrot,DECBINT);
                    if(Exceptions) { DSTop=expression; LAMTop=lamsave; nLAMBase=lamcurrent; return 0; }
                    // GET LEFT/RIGHT INDEX FROM PARENT
    ...
    case ARGDONE:
    {
        ...
        if(leftidx<=leftnargs) {
            if(rightidx>rightnargs) {
                ...
                if( (**left==CMD_OVR_ADD)||(**left==CMD_OVR_MUL)) {
