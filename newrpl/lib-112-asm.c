...
BYTEPTR rplAsmEndOfToken(BYTEPTR start,BYTEPTR end)
{
    ...
    if((start<end) && ((*start=='+')||(*start=='-')||(*start=='*')||(*start=='/')||(*start=='^')||(*start=='='))) {
        // THE TOKEN IS AN OPERATOR
        while(start<end) {
            if((*start!='+')&&(*start!='-')&&(*start!='*')&&(*start!='/')&&(*start!='^')&&(*start!='=')) break;
    ...
    while(start<end) {
        if((*start=='.')||(*start=='+')||(*start=='-')||(*start=='*')||(*start=='/')||(*start=='^')||(*start=='=')) break;
        start=(BYTEPTR)utf8skipst((char *)start,(char *)end);
    }
    }
    return start;
...
BINT rplAsmDecodeToken(BYTEPTR start,BYTEPTR end,BINT *opcode_arg)
{
    ...
    if(*start=='.') ++start;    // SKIP THE SEPARATOR
    ...
    if((end-start==5)&&(start[4]=='H')) {
        ...
        while(tbl->opname) {
            ...
            if(tbl->opname==name) { *opcode_arg=tbl->opcode; return 1; }
            ++tbl;
        }
    }
    else {
...
void LIB_HANDLER()
{
    ...
    case ASM_LDY:
    {
        ...
        rplPushDataNoGrow(argY);
        DSTop[-2]=*destD;
        if(ISNUMBERCPLX(*argY) && ISNUMBERCPLX(**destD)) rplCallOvrOperator(opcode);
    ...
    {
        if(LIBNUM(CurOpcode)!=LIBRARY_NUMBER) return;   // IGNORE ALL OTHER ASSOCIATED LIBRARY NUMBERS
        BYTEPTR str=(BYTEPTR)TokenStart,endtoken;

        if(*str!=':') { RetNum=ERR_NOTMINE; return; }       // MUST START WITH A COLON

        ++str;

        while((*str!=':')&&(str<(BYTEPTR)BlankStart)) ++str;    // MUST NOT CONTAIN ANY OTHER COLON

        if(str!=(BYTEPTR)BlankStart) { RetNum=ERR_NOTMINE; return; }
        ...
        if(!rplAsmDecodeToken(str,endtoken,&opcode_arg)) { RetNum=ERR_SYNTAX; return; }
        ...
        if(opcode_arg>>16) { RetNum=ERR_SYNTAX; return; }   // CANNOT START WITH ASSIGNMENT
        if(operator && (operator<=ASM_POW)) { RetNum=ERR_SYNTAX; return; } // BINARY OPERATORS CANNOT BE AT THE BEGINNING
        ...
        if(!rplAsmDecodeToken(str,endtoken,&opcode_arg)) {
            if(str!=(BYTEPTR)BlankStart) { RetNum=ERR_SYNTAX; return; }
            ...
            if(!storemode) {
                ...
                if(d&0x20) { RetNum=ERR_SYNTAX; return; }   // STORAGE OPERATOR WITHOUT DESTINATION
                ...
                if(!rplAsmDecodeToken(str,endtoken,&opcode_arg)) {
                    RetNum=ERR_SYNTAX; return;
                }

                if(opcode_arg&~0xffff) { RetNum=ERR_SYNTAX; return; }   // CANNOT BE A STORAGE OPERATOR AGAIN

                // IT CAN BE AN OPERATOR OR THE ARGUMENT Y
                y=opcode_arg&0xff;
                operator=(opcode_arg>>8)&0xff;

                if(operator && (operator<=ASM_POW)) { RetNum=ERR_SYNTAX; return; } // BINARY OPERATOR BEFORE THE ARGUMENT
                ...
                if(!rplAsmDecodeToken(str,endtoken,&opcode_arg)) {
                    ...
                    if(!operator) {
                        operator=(opcode_arg>>8)&0xff;
                        if(!operator || (operator>ASM_POW)) { RetNum=ERR_SYNTAX; return; } // NOT A BINARY OPERATOR!
                    }
                    ...
                    if(y&0x20) {
                        ...
                        if(!rplAsmDecodeToken(str,endtoken,&opcode_arg)) {
                            ...
                            if(endtoken!=(BYTEPTR)BlankStart) { RetNum=ERR_SYNTAX; return; }   // SHOULD BE THE LAST ARGUMENT
                        }
                        ...
                        if(!(opcode_arg&0x20)) {
                        z=opcode_arg&0xff;
                        if(opcode_arg&~0xff) { RetNum=ERR_SYNTAX; return; }   // CANNOT BE AN OPERATOR OR STORAGE, MUST BE AN ARGUMENT
                        if(endtoken!=(BYTEPTR)BlankStart) { RetNum=ERR_SYNTAX; return; }   // SHOULD BE THE LAST ARGUMENT
                        }
                        else {
                            // MOVE ON TO THE NEXT ARGUMENT
    ...
    case OPCODE_DECOMPILE:
    {
        ...
        // RetNum =  enum DecompileErrors
         ...
         if(storemode) {
             BINT d=(op>>8)&0xf;
             if(d==8) rplDecompAppendChar('P');
             else if(d>8) {
                 rplDecompAppendChar('S');
                 rplDecompAppendChar('0'+d-8);
             } else rplDecompAppendChar('A'+d);
         ...
         if(!noy) {
             ...
             if(op&0x2000) {
              // Y IS A LITERAL VALUE
                 ...
                 rplDecompAppendChar('#');
                 if(y<10) rplDecompAppendChar('0'+y);
                 else { rplDecompAppendChar('1'); rplDecompAppendChar('0'+y-10); }
                 }
             }
             else {
                 // IT'S A REFERENCE
             if(y==8) rplDecompAppendChar('R');
             else if(y>8) {
                 rplDecompAppendChar('S');
                 rplDecompAppendChar('0'+y-8);
             } else rplDecompAppendChar('A'+y);
             }
             ...
             } else  if(!noz) rplDecompAppendChar('.');
             ...
             if(!noz) {
                 ...
                 if(op&0x1000) {
                  // Z IS A LITERAL VALUE
                     ...
                     // LITERAL VALUE IN NUMERIC FORM
                     rplDecompAppendChar('#');
                     if(z<10) rplDecompAppendChar('0'+z);
                     else { rplDecompAppendChar('1'); rplDecompAppendChar('0'+z-10); }

                 }
                 else {
                     // IT'S A REFERENCE
                 if(z==8) rplDecompAppendChar('R');
                 else if(z>8) {
                     rplDecompAppendChar('S');
                     rplDecompAppendChar('0'+z-8);
                 } else rplDecompAppendChar('A'+z);
                 }
