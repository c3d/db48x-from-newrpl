...
// ANGLES ARE BASIC OBJECTS AND SHOULD STAY ANGLES AFTER ->NUM
            ...
            if(ISANGLE(*arg1) && ISANGLE(*arg2)) {
                ...
                if((ISANGLE(*arg1) || ISNUMBER(*arg1)) && (ISANGLE(*arg2)
                            || ISNUMBER(*arg2))) {
                    if(ISANGLE(*arg1) || ISANGLE(*arg2)) {
...
#undef arg2
    ...
    case ANGTODEG:
    {
        ...
        if(ISCOMPLEX(*rplPeekData(1))) {
            ...
            if(angmode == ANGLEDEG)
                return; // ALREADY IN THE RIGHT SYSTEM
        ...
        if(ISMATRIX(*rplPeekData(1))) {
            ...
            for(f = 0; f < cols; ++f) {
                ...
                if(ISCOMPLEX(*first[f])) {
                    ...
                    if(angmode == ANGLEDEG)
                        continue;       // ALREADY IN THE RIGHT SYSTEM
        ...
        // CONVERT EITHER A COMPLEX OR A VECTOR TO POLAR COORDINATES
    {
        ...
        if(ISMATRIX(*rplPeekData(1))) {
            ...
            if(rplMatrixIsPolar(matrix)) {
                ...
                if(template == 1)
                    return;     // NOTHING TO DO, ALREADY POLAR
        ...
        // CONVERT EITHER A COMPLEX OR A VECTOR TO POLAR COORDINATES
    {
        ...
        if(ISMATRIX(*rplPeekData(1))) {
            ...
            if(rplMatrixIsPolar(matrix)) {
                ...
                if(template == sphere)
                    return;     // NOTHING TO DO, ALREADY SPHERICAL
    ...
    case OPCODE_CHECKOBJ:
        ...
        if(ISPROLOG(*ObjectPTR)) {
            ...
            if(rplSkipOb(ObjectPTR) != rplSkipOb(ObjectPTR + 1)) {
                RetNum = ERR_INVALID;
                return;
            }
            if(!ISNUMBER(ObjectPTR[1])) {
                RetNum = ERR_INVALID;
                return;
            }
