...
int32_t rplStringGetLinePtr(word_p str, int32_t line)
{
    ...
    if(ptr - start >= len)
        return -1;      // THIS CAN ONLY HAPPEN ON STRINGS TERMINATED IN A NEWLINE WHEN THE NEXT LINE IS REQUESTED
    return ptr - start;
}
...
byte_p rplSkipSep(byte_p start, byte_p end, byte_p sepstart, byte_p sepend)
{
    ...
    while(start != end) {
        ...
        while(sepptr != sepend) {
            ...
            if(sepptr == sepnext) {
                sepptr = sepstart;      // THERE WAS A MATCH!
                break;
            }
            // NO MATCH, KEEP GOING
            sepptr = sepnext;
        ...
        if(sepptr == sepend)
            return start;       // THERE WAS NO MATCH
...
byte_p rplNextSep(byte_p start, byte_p end, byte_p sepstart, byte_p sepend)
{
    ...
    while(start != end) {
        ...
        while(sepptr != sepend) {
            ...
            if(sepptr == sepnext)
                return start;   // THERE WAS A MATCH!
            // NO MATCH, KEEP GOING
            sepptr = sepnext;
        }
        // THERE WAS NO MATCH
...
void LIB_HANDLER()
{
    ...
    case TOUTF:
    {
        ...
        word_p newstring = rplAllocTempOb(nitems);     // ALLOCATE 4 BYTES PER CHARACTER, TRUNCATE THE OBJECT LATER
        if(!newstring)
            return;

        byte_p strptr = (byte_p) (newstring + 1);     // START OF NEW STRING AFTER THE PROLOG
        ...
        while(nitems--) {
            ...
            len = (utfchar & 0xffff0000) ? ((utfchar & 0xff000000) ? 4 : 3)
                    : ((utfchar & 0xff00) ? 2 : 1);
            while(len--) {
                *strptr = utfchar & 0xff;
                ++strptr;
    ...
    case FROMUTF:
    {
        ...
        while(ScratchPointer1 <= ScratchPointer2) {
            ...
            ScratchPointer1 =
                    (word_p) utf8skip((char *)ScratchPointer1,
                    ((char *)ScratchPointer2) + 1);
        }
        ...
        // VERY IMPORTANT: DECOMPILE FUNCTION
    {
        ...
        word_p string = rplDecompile(rplPeekData(1), flag ? DECOMP_EDIT : 0);
    ...
    case TODISPSTR:
        // VERY IMPORTANT: DECOMPILE FUNCTION
    {
        //@SHORT_DESC=Decompile formatted for display
        if(rplDepthData() < 1) {
            rplError(ERR_BADARGCOUNT);
            return;
        }

        word_p string = rplDecompile(rplPeekData(1), 0);
    ...
    case TOEDITSTR:
        // VERY IMPORTANT: DECOMPILE FUNCTION
    {
        //@SHORT_DESC=Decompile formatted for edit
        if(rplDepthData() < 1) {
            rplError(ERR_BADARGCOUNT);
            return;
        }

        word_p string = rplDecompile(rplPeekData(1), DECOMP_EDIT);
    ...
    case SREV:
    {
        ...
        word_p newobj = rplAllocTempOb(OBJSIZE(*rplPeekData(1)));
        if(!newobj)
            return;

        word_p oldobj = rplPeekData(1);
    ...
    case NTHTOKEN:
    {
        ...
        rplDropData(3);
        word_p newstring =
                rplCreateString(strstart, rplNextSep(strstart, strend, sepstart,
                    sepend));
        if(!newstring)
            return;
    ...
    case TRIM:
    {
        ...
        while((strstart != strend)) {
            ...
            strstart = rplSkipSep(strstart, strend, sepstart, sepend);
            if(strstart == strend)
                break;  // STRING ENDS IN BLANKS
            nextsep = rplNextSep(strstart, strend, sepstart, sepend);
            if(nextsep == strend) {
                lastsep = strend;       // STRING DOESN'T END IN BLANKS
                break;
            }
            strstart = nextsep;
        }
    ...
    case RTRIM:
    {
        ...
        word_p newstr = rplCreateString(firsttok, strend);
        if(!newstr)
            return;
    ...
    case SREPL:
    {
        ...
        for(; pos <= maxpos; ++pos) {
            if(utf8ncmp2((char *)nextchar, (char *)end1, (char *)find,
                ...
                if(((newsize2 + 3) >> 2) > ((newsize + 3) >> 2)) {
                    ...
                    ScratchPointer1 = (word_p) str1;
                    ScratchPointer2 = (word_p) find;
                    ScratchPointer3 = (word_p) repl;
                    ScratchPointer4 = newstring;
                    rplResizeLastObject(((newsize2 + 3) >> 2) - ((newsize +
                                    3) >> 2));
                    if(Exceptions)
                        return;
        ...
        if(rcount) {
            memmoveb(((byte_p) (newstring + 1)) + newsize, str1, end1 - str1);
        ...
        // APPEND TWO STRINGS
    {
        ...
        if(!ISSTRING(*rplPeekData(2))) {
            str1 = rplDecompile(rplPeekData(2),
                    flag ? (DECOMP_EDIT | DECOMP_NOHINTS) : DECOMP_NOHINTS);
            if(!str1) {
                DSTop = savestk;        // THERE WAS AN ERROR, TAKE OWNERSHIP OF IT
        ...
        if(!ISSTRING(*rplPeekData(2))) {
            str2 = rplDecompile(rplPeekData(2),
                    flag ? (DECOMP_EDIT | DECOMP_NOHINTS) : DECOMP_NOHINTS);
            if(!str2) {
                DSTop = savestk;        // THERE WAS AN ERROR, TAKE OWNERSHIP OF IT
                ExceptionPointer = IPtr;
        ...
        word_p newobject = rplAllocTempOb((len1 + len2 + 3) >> 2);
        if(!newobject) {
        ...
        memmoveb(((byte_p) newobject) + len1 + 4, str2 + 1, len2);
    ...
    case OVR_EQ:
        ...
        if((!ISSTRING(*rplPeekData(2))) || (!ISSTRING(*rplPeekData(1)))) {
            rplOverwriteData(2, (word_p) zero_bint);
            rplDropData(1);
            return;
        }

        if(rplStringCompare(rplPeekData(1), rplPeekData(2)))
        ...
        if(*((byte_p) TokenStart) == '\"') {
            ...
            do {
                while(count < 4) {
                    ...
                    if(escape > 1 && ((*ptr >= '0') && (*ptr <= '9'))) {
                        ++escape;
                        code = code * 16 + (*ptr - '0');
                        ++ptr;
                        continue;
                    }
                    if(escape > 1 && ((*ptr >= 'A') && (*ptr <= 'F'))) {
                        ++escape;
                        code = code * 16 + (*ptr - 'A' + 10);
                        ++ptr;
                        continue;
                    }
                    if(escape > 1 && ((*ptr >= 'a') && (*ptr <= 'f'))) {
