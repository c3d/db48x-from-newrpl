...
void LIB_HANDLER()
{
    ...
    if(LIBNUM(CurOpcode) == LIB_OVERLOADABLE) {
        ...
        case OVR_ADD:
        {
            ...
            if(isspec1 && isspec2)
                rplUnitSpecialToDelta(nlevels1);
            else {
                if((isspec1 + isspec2 > 0) && (isspec1 + isspec2 < 7))
            ...
            // FIRST, DO A SUBTRACTION
        {
            ...
            if(isspec1)
                ...
                if(ISUNIT(*unitval))
                    ++unitval;  // IF IT'S A UNIT, POINT TO THE VALUE
                rplPushData(unitval);
            }
            rplPushData(rplPeekData(nlevels2 + 1));
            rplPushData(rplPeekData(nlevels1 + nlevels2 + 2));
                ...
                if(needident) {
                    ...
                    if(*nextptr == ')') {
                        ...
                        if(*(nextptr + 1) == '^') {
                            ...
                            if(*(nextptr + 1) == '(') {
                                ...
                                newRealFromText(&RReg[0], (char *)nextptr,
                                        (char *)numend, Locale);

                                if(RReg[0].
                                        flags & (F_ERROR | F_INFINITY |
                                            F_NOTANUMBER | F_NEGUNDERFLOW |
                                            F_POSUNDERFLOW | F_OVERFLOW)) {
                                    // BAD EXPONENT!
                                ...
                                if(*nextptr == ')') {
                                    ...
                                    newRealFromText(&RReg[1], (char *)nextptr,
                                            (char *)numend, Locale);
                            ...
                            groupptr =
                                    *(ValidateTop - 1) + groupoff[groupidx - 1];
                            ...
                            while(offset < groupsize) {
                                ...
                                groupptr =
                                        *(ValidateTop - 1) + groupoff[groupidx -
                                        1];
                                ...
                                memmovew(CompileEnd - unitlen, unitptr,
                                        unitlen);
                            ...
                            memmovew(groupptr, groupptr + groupsize,
                                    CompileEnd - (groupptr + groupsize));
                    ...
                    if(*nextptr == '^') {
                        ...
                        if(*(nextptr + 1) == '(') {
                            ...
                            if(RReg[0].
                                    flags & (F_ERROR | F_INFINITY | F_NOTANUMBER
                                        | F_NEGUNDERFLOW | F_POSUNDERFLOW |
                                        F_OVERFLOW)) {
                                // BAD EXPONENT!
                                rplError(ERR_EXPECTEDREALEXPONENT);
                                RetNum = ERR_SYNTAX;
    ...
    case OPCODE_DECOMPILE:
        ...
        if(IS_PROLOG(*DecompileObject)) {
            ...
            while(offset < totalsize) {
                ...
                if(!((rden.len == 1) && (rden.data[0] == 1) && (rnum.len == 1)
                            && (rnum.data[0] == 1))) {
                    rplDecompAppendChar('^');
