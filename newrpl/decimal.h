...
#include <stdlib.h>
#include "recorder.h"
#define PROTECT_WRITE_AREA(ptr,len) { if( ((ptr)<Context.regdata) || ((((word_p)ptr)+(len))>=(word_p)Context.regdata+REAL_REGISTER_STORAGE*TOTAL_REGISTERS)) { printf("PANIC EXIT-BAD WRITE - Attach debugger\n"); int debug = 1; while (debug); } }
#endif
...
typedef struct
{
    ...
    int32_t precdigits;
    WORD alloc_bmp;
    int32_t regdata[REAL_REGISTER_STORAGE * TOTAL_REGISTERS];
} DECIMAL_CONTEXT;
...
int32_t trailzerosReal(REAL * num);
...
// RANGE SO IT CAN BE CONVERTED TO int32_t OR int64_t WITHOUT
// OVERFLOW.
...
#define MAKELOCALE(dec,thousand,frac,arg) ((((uint64_t)((dec)&0xffff))<<0)|(((uint64_t)((thousand)&0xffff))<<16)|(((uint64_t)((frac)&0xffff))<<32)|(((uint64_t)((arg)&0xffff))<<48))
