...
void FSHardReset()
{
    ...
    if(!SDCardInserted())
        SDIOSetup(NULL, 1);     // POWER OFF SD CONTROLLER IF NO CARD INSERTED
}
...
int FSInit()
{
    ...
    if(!SDCardInit(dsk)) {
        FSystem.CurrentVolume = dsk->SysFlags;
        simpfree(fs);
        simpfree(dsk);
        return FS_NOCARD;
    }
...
int FSCardIsSDHC()
{
    if(!FSystem.Init)
        return 0;
    if(FSystem.Volumes[FSystem.CurrentVolume]->Disk->SysFlags & 16)
        return 1;
    return 0;
}
...
int FSIsDirty()
{
    ...
    for(vol = 0; vol < 4; ++vol) {
        if(FSystem.Volumes[vol]) {
            if(FSystem.Volumes[vol]->NumCache)
                return 1;       // VOLUME NEEDS TO UPDATE THE FAT
            if((FSystem.Volumes[vol]->
                        InitFlags & (VOLFLAG_UPDATEHINT | VOLFLAG_HINTDIRTY)) ==
                    (VOLFLAG_UPDATEHINT | VOLFLAG_HINTDIRTY))
                return 1;       // VOLUME NEEDS TO UPDATE ITS HINTS
            int f;
            for(f = 0; f < FS_MAXOPENFILES; ++f) {
                if(FSystem.Volumes[vol]->Files[f] != NULL) {
                    if(FSystem.Volumes[vol]->Files[f]->Mode & FSMODE_WRITE)
                        return 2;       // VOLUME HAS OPEN FILES FOR WRITING
                }
            }
        }
    }
