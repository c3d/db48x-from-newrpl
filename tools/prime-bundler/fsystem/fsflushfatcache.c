...
// ENTRY IS PARTIALLY WITHIN BLOCKS - ONLY FAT12
            if(ch->Entries[0].EntryValue & 0x40000000)
                ++sectaddr;
        }
        sectaddr &= ~bl;

        error = SDDRead((((uint64_t) fs->FirstFATAddr) << 9) + sectaddr, bl + 1,
                sector, fs->Disk);
        if(error != bl + 1) {
            simpfree(sector);
            return FS_ERROR;
...
// WRITE TO ALL ACTIVE FATS
        ...
        for(f = fs->NumFATS - 1; f >= 0; --f) {
            ...
            error = SDDWrite((((uint64_t) fs->FirstFATAddr) << 9) + sectaddr +
                    f * (fs->FATSize << fs->SectorSize), bl + 1, sector,
                    fs->Disk);
        ...
        if(FSystem.Volumes[vol]) {
            ...
            for(k = 0; k < FS_MAXOPENFILES; ++k) {
                if(FSystem.Volumes[vol]->Files[k]) {
                    ...
                    if(FSystem.Volumes[vol]->Files[k]->Mode & FSMODE_WRITE)
                        FSFlushBuffers(FSystem.Volumes[vol]->Files[k]);
                }
            }
