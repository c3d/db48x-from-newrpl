...
// FRAGMENT FITS EXACTLY
        if(FSWriteFATEntry(FSAddr2Cluster(ch->EndAddr, fs) - 1, 0xfffffff,
                    fs) != FS_OK)
            return FS_ERROR;    // MARK LAST CLUSTER AS END-OF-CHAIN
...
// TRUNCATE CHAIN
            ...
            if(newsize) {
                if(FSWriteFATEntry(FSAddr2Cluster(addr, fs) - 1, 0xfffffff,
                            fs) != FS_OK)
                    return FS_ERROR;    // MARK LAST CLUSTER AS END-OF-CHAIN
            }
            else {
            ...
            while(addr != ch->EndAddr) {
                if(FSWriteFATEntry(FSAddr2Cluster(addr, fs), 0, fs) != FS_OK)
                    return FS_ERROR;    // MARK CLUSTERS AS FREE
                addr += clsize;
            }
