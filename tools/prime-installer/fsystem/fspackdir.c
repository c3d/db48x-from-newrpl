...
int FSPackDir(FS_FILE * dir)
{
    ...
    while((checksum = FSReadLL(buffer, 32, dir, fs)) == 32) {
        ...
        if(((buffer[11] & FSATTR_LONGMASK) == FSATTR_LONGNAME) &&
                ((fs->FATType == 3) || (((buffer[0x1a] | buffer[0x1b]) == 0)
...
//      printf("All valid!!!\n");
            ...
            FSMovedOpenFiles(dir, diroffset,
                    bufoffset + (int)(buffer - packbuf), fs);
...
// SAVE PARTIAL BUFFER IF FULL
    ...
    if(!((dir == &fs->RootDir) && (fs->FATType != 3))) {
        ...
        if(FSTruncateChain(dir, bufoffset + (int)(buffer - packbuf)) == FS_OK) {
            ...
            if(dir->FileSize < 65536 * 32)
                dir->Mode &= ~FSMODE_NOGROW;    // ALLOW DIRECTORY TO GROW
        }
        else {
            simpfree(packbuf);
            return FS_ERROR;
...
// CLEAN UNUSED ENTRIES
        ...
        if(dir->FileSize < bufoffset + bufsize)
            savesize = dir->FileSize - bufoffset;
