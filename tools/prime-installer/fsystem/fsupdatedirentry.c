...
int FSUpdateDirEntry(FS_FILE * file)
{
    ...
    if(FSReadLL(buffer, file->DirEntryNum << 5, file->Dir,
                FSystem.Volumes[file->Volume]) != file->DirEntryNum << 5) {
        simpfree(buffer);
        return FS_ERROR;
    }
...
//mainentry[12]=file->NTRes;
    ...
    if(FSystem.Volumes[file->Volume]->FATType == 3)
    ...
    if(FSWriteLL(buffer, file->DirEntryNum << 5, file->Dir,
                FSystem.Volumes[file->Volume]) != file->DirEntryNum << 5) {
        simpfree(buffer);
        return FS_ERROR;
    }
