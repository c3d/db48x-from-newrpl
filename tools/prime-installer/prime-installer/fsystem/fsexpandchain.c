...
// NUMBER OF NEW CLUSTERS TO ALLOCATE
    needed = (((uint64_t) newtotalsize - (((uint64_t) size) << 9)) +
            (1 << fs->ClusterSize) - 1) >> fs->ClusterSize;
    if(needed <= 0)
        return FS_OK;   // NO NEED TO ADD CLUSTERS TO THE CHAIN

    needed <<= (fs->ClusterSize - 9);
//printf("needed=%d\n",needed);
    if((fs->InitFlags & VOLFLAG_FREESPACEVALID)
            && (fs->FreeSpace < (unsigned int)needed))
        return FS_DISKFULL;
...
// IF CALCFREESPACE FAILS TO READ THERE'S A BIG PROBLEM, ABORT
            int tmperr =
                    FSScanFreeSpace(fs, FSAddr2Cluster(fs->NextFreeCluster,
                        fs));
            if(tmperr != FS_OK)
                return tmperr;
        }

        if(fs->FreeAreaSize > (unsigned int)needed)
            taken = needed;
        else
            taken = fs->FreeAreaSize;
...
// WRITE FAT ENTRIES
            while(fixcluster != fr->EndAddr) {
                FSWriteFATEntry(FSAddr2Cluster(fixcluster, fs) - 1,
                        FSAddr2Cluster(fixcluster, fs), fs);
...
// LINK OLD CHAIN W/NEW FRAGMENT
                FSWriteFATEntry(FSAddr2Cluster(fr->EndAddr, fs) - 1,
                        FSAddr2Cluster(newfr->StartAddr, fs), fs);
            }
            fr = newfr;
//printf("st=%08X, e=%08X\n",fr->StartAddr,fr->EndAddr);
