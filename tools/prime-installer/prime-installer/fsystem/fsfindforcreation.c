...
// INITIALIZE COUNTERS
    ...
    if(dir->Mode & FSMODE_NOGROW)
        cr->DirMaxEntries = dir->FileSize >> 5;
    else
...
// SAFE OBTAIN NAME OF NEW FILE
    if(namelen > 255) {
        ...
        stringcpy((char *)newname, (char *)name);

// STRIP SEMICOLONS IF PRESENT
    if(FSystem.CaseMode == FSCASE_SENSHP
...
// EXTRACT ROOT NAME TO CHECK FOR CONFLICTS
    ...
    if(cr->NameFlags & 1)
        cr->ShortNum = 1;       // MINIMUM NUMBER NEEDED =1
...
//      printf("last entry\n");
            ...
            if((nbytesread =
                        FSReadLL(morebuff, 32 * nentries, dir,
                            fs)) != 32 * nentries) {
                simpfree(morebuff);
                if(nbytesread > 0)
                    cr->DirUsedEntries += nbytesread >> 5;
...
//      printf("All valid!!!\n");
            ...
            for(order = 1; order < nentries; ++order) {
                entrynameptr =
                        (unsigned char *)FSPackName((char *)entrynameptr,
                        (char *)ptr - (order << 5));
            }
            entrynameptr =
                    (unsigned char *)FSPackName((char *)entrynameptr,
            ...
            *entrynameptr = 0;  // FORCE NULL-TERMINATED STRING
            ...
            int compare =
                    FSNameCompare((char *)entryname, (char *)newname,
                    (FSystem.CaseMode ==
                        FSCASE_SENS) ? FSCASE_INSENS : FSystem.
                    CaseMode) | (FSNameCompare((char *)shortn, (char *)newname,
                        (FSystem.CaseMode ==
                            FSCASE_SENS) ? FSCASE_INSENS : FSystem.
                        CaseMode) << 1);
            ...
            if(compare & 1) {
                ...
                if(!cr->FileExists)     // DON'T CHECK FOR DUPLICATED FILES
                {
                    ...
                    cr->Entry->Dir = dir;
                    memsetb((void *)&(cr->Entry->Chain), 0,
                            sizeof(FS_FRAGMENT));
                    memsetb((void *)&(cr->Entry->RdBuffer), 0,
                            sizeof(FS_BUFFER));
                    memsetb((void *)&(cr->Entry->WrBuffer), 0,
                            sizeof(FS_BUFFER));
                }
            ...
            else        // IF NOT EXACT MATCH, CHECK FOR APPROXIMATE MATCHES
            {
                ...
                if(FSystem.CaseMode != FSCASE_INSENS) {
                    if(FSNameCompare((char *)entryname, (char *)newname,
                                (FSystem.CaseMode ==
                                    FSCASE_SENS) ? FSCASE_INSENS :
                                FSCASE_INSENSHP)) {
                        ...
                        while(*tmp)
                            ++tmp;
                        --tmp;
                        while((*tmp == ';') && (tmp >= entryname)) {
                            tmp--;
                            ++semis;
                        }

                        if(cr->NeedSemi <= semis)
                            cr->NeedSemi = semis + 1;
                ...
                if(FSystem.CaseMode != FSCASE_INSENS) {
                    if(FSNameCompare((char *)shortn, (char *)newname,
                                (FSystem.CaseMode ==
                                    FSCASE_SENS) ? FSCASE_INSENS :
                                FSCASE_INSENSHP)) {
                        // CASE CONFLICT - DETERMINE HOW MANY SEMICOLONS TO ADD
                ...
                {
                    int rootn =
                            FSNameCompareRoot((char *)shortn, (char *)shname);
    ...
    if(cr->NeedSemi)
        cr->NameFlags |= 3;     // FORCE LONG NAME W/SHORT NAME TAIL
...
// REPACK IF MORE THAN 100 ENTRIES TOTAL AND (BAD_ENTRIES>=VALID_ENTRIES)
    if(((cr->DirUsedEntries - cr->DirValidEntries > cr->DirValidEntries >> 1)
                && (cr->DirUsedEntries > 100))
            ...
            || ((cr->DirUsedEntries - cr->DirValidEntries > 0)
                && (cr->DirMaxEntries - cr->DirUsedEntries < 100))) {
