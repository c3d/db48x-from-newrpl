...
//if(!SDDSetBlockLen(fs->Disk,7)) { simpfree(buffer); return FS_ERROR; }
    ...
    do {
        ...
        if(SDDRead((((uint64_t) fs->FirstFATAddr) << 9) + fataddr, 1536, buffer,
                    fs->Disk) != 1536) {
            simpfree(buffer);
            return FS_ERROR;
        }
...
// SCAN FAT BLOCK
        if(maxaddr - fataddr < 1536)
            limit = maxaddr - fataddr;
        else
            limit = 1536;
...
// FAT 12 SCANNING
            for(f = 0; f < limit; f += 3) {
                value = buffer[f] + (buffer[f + 1] << 8) + (buffer[f +
                            2] << 16);
                if(!value) {
                    ...
                    if(value & 0xfff) {
                        if(freestart && (fs->FreeAreaSize < freesize)) {
                            fs->NextFreeCluster = freestart;
                            fs->FreeAreaSize = freesize;
                        }
                        freestart = 0;
                    }
                    else {
...
// PROCESS 2 CLUSTERS AT A TIME
        ...
        if(fataddr >= maxaddr)
            fataddr = 0;        // WRAP AROUND AND START OVER
    ...
    if(fs->NextFreeCluster != initialnextfree)
