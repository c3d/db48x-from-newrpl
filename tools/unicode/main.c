...
void processExclusions()
{
    ...
    while(cp_ptr != NULL) {
        ...
        if((cp_ptr->decomp[0] != -1) && (cp_ptr->decomp[1] == -1))
            cp_ptr->exclusion = 1;

        // EXCLUDE ALL NON-STARTER DECOMPOSITIONS
        if((cp_ptr->cc != 0) && (cp_ptr->decomp[0] != -1))
            cp_ptr->exclusion = 1;
...
void doNFCTest(int line)
{
    ...
    while(*ptr) {
        ...
        int k = 0;
        while(unicodeBuffer[k])
            intAppend(tmp, unicodeBuffer[k++]);
    }
...
void packClassData()
{
    ...
    while(ptr != NULL) {
        ...
        if(k == n) {
            classes[n++] = ptr->cc;
            --k;
            while(k >= 0 && classes[k] > classes[k + 1]) {
                int tmp = classes[k + 1];
    ...
    while(ptr != NULL) {
        ...
        int pack =
                j | ((ptr->nfc_qc) ? 0x40 : 0) | ((ptr->exclusion) ? 0x80 : 0);
    ...
    do {
        r = j + 1;
        while((packedData[r] == packedData[j]) && (r < 0x110000))
            ++r;
        if(r - j > PACK_THRESHOLD) {
            if(j != prevrange) {
                ...
                while(j - prevrange > MAX_NCHARS) {
                    ...
                    if(location < 0) {
                        ...
                        for(f = prevrange; f < prevrange + MAX_NCHARS;
                                ++f, ++used_CCBytes)
                            packed_CCBytes[used_CCBytes] = packedData[f];
                    }
                    else {
                ...
                int location =
                        searchCCBytes(packedData + prevrange, j - prevrange);
                unsigned int data;
                printf("Range: %04X..%04X, LEN=%d --> OFFSET=%d\n", prevrange,
                        j - 1, j - prevrange,
                        location < 0 ? used_CCBytes : location);
                if(location < 0) {
                    ...
                    int f;
                    for(f = prevrange; f < j; ++f, ++used_CCBytes)
                        packed_CCBytes[used_CCBytes] = packedData[f];
                }
                else {
...
void packDecompositions()
{
    ...
    while(ptr != NULL) {
        if(ptr->decomp[0] != -1) {
            if(ptr->decomp[1] != -1) {
                ...
                if((int)ptr->decomp[1] - (int)ptr->decomp[0] > max_dbldiff)
                    max_dbldiff = ptr->decomp[1] - ptr->decomp[0];
                if((int)ptr->decomp[1] - (int)ptr->decomp[0] < min_dbldiff)
                    min_dbldiff = (int)ptr->decomp[1] - (int)ptr->decomp[0];
    ...
    if(singleton_from[j] != 0) {
        ...
        printf("Range %04X..%04X, LEN=%d ***GAP***\n", 0, singleton_from[j] - 1,
                singleton_from[j]);
        ++countrange;

        packed_singletonRanges[used_singletonRanges] =
                MK_SINGGAP(0, singleton_from[j] - 1);
    ...
    do {
        r = j + 1;
        while((singleton_from[r] == singleton_from[r - 1] + 1)
                && (r < used_sing))
            ++r;
        ...
        if(r < used_sing) {
            ...
            if(gapend - gapstart + 1 < SING_THRESHOLD) {
                ...
                printf("Range %04X..%04X, LEN=%d \n", singleton_from[prevrange],
                        singleton_from[r - 1],
                        singleton_from[r - 1] - singleton_from[prevrange] + 1);
                totalsing +=
                        singleton_from[r - 1] - singleton_from[prevrange] + 1;
                ++countrange;

                packed_singletonRanges[used_singletonRanges] =
                        MK_SINGRANGE(singleton_from[prevrange],
                        singleton_from[r - 1], used_singletonData);
                ++used_singletonRanges;
                int f;
                ...
                for(f = 0;
                        ...
                        singleton_from[r - 1] - singleton_from[prevrange] + 1;
                        ++f)
                    packed_singletonData[used_singletonData + f] = -1;

                // NOW FILL THE CORRESPONDING VALUES
                for(f = 0; f < r - prevrange; ++f) {
                    packed_singletonData[used_singletonData +
                            singleton_from[prevrange + f] -
                            singleton_from[prevrange]] =
                            singleton_to[prevrange + f];
                }
                used_singletonData +=
                        singleton_from[r - 1] - singleton_from[prevrange] + 1;
            ...
            // FIRST FILL THE TABLE WITH -1
            for(f = 0;
                    f < singleton_from[r - 1] - singleton_from[prevrange] + 1;
                    ++f)
                packed_singletonData[used_singletonData + f] = -1;
            ...
            if(singleton_from[r - 1] < 0x10ffff) {
                printf("Range %04X..%04X, LEN=%d ***GAP***\n",
                        singleton_from[r - 1] + 1, 0x10ffff,
                        0x10ffff - singleton_from[r - 1]);
                ++countrange;

                packed_singletonRanges[used_singletonRanges] =
                        MK_SINGGAP(singleton_from[r - 1] + 1, 0x10ffff);
                ++used_singletonRanges;
            }
    ...
    if(doubles_from[j] != 0) {
        ...
        packed_doubleRanges[used_doubleRanges] =
                MK_SINGGAP(0, doubles_from[j] - 1);
        ++used_doubleRanges;
    ...
    do {
        r = j + 1;
        while((doubles_from[r] == doubles_from[r - 1] + 1)
                && (r < used_doubles))
            ++r;
        ...
        if(r < used_doubles) {
            ...
            if(gapend - gapstart + 1 < DBL_THRESHOLD) {
                ...
                printf("Range %04X..%04X, LEN=%d \n", doubles_from[prevrange],
                        doubles_from[r - 1],
                        doubles_from[r - 1] - doubles_from[prevrange] + 1);
                totaldbl += doubles_from[r - 1] - doubles_from[prevrange] + 1;
                ++countrange;

                packed_doubleRanges[used_doubleRanges] =
                        MK_SINGRANGE(doubles_from[prevrange],
                        doubles_from[r - 1], used_doubleData);
                ++used_doubleRanges;
                int f;
                // FIRST FILL THE TABLE WITH -1
                for(f = 0;
                        f < doubles_from[r - 1] - doubles_from[prevrange] + 1;
                        ++f) {
                ...
                // NOW FILL THE CORRESPONDING VALUES
                for(f = 0; f < r - prevrange; ++f) {
                    packed_doubleData[2 * (used_doubleData +
                                (doubles_from[prevrange + f] -
                                    doubles_from[prevrange]))] =
                            doubles_to1[prevrange + f];
                    packed_doubleData[2 * (used_doubleData +
                                (doubles_from[prevrange + f] -
                                    doubles_from[prevrange])) + 1] =
                            doubles_to2[prevrange + f];
            ...
            packed_doubleRanges[used_doubleRanges] =
                    MK_SINGRANGE(doubles_from[prevrange], doubles_from[r - 1],
                    used_doubleData);
            ++used_doubleRanges;
            int f;
            // FIRST FILL THE TABLE WITH -1
            for(f = 0; f < doubles_from[r - 1] - doubles_from[prevrange] + 1;
                    ++f) {
                packed_doubleData[2 * (used_doubleData + f)] = -1;
                packed_doubleData[2 * (used_doubleData + f) + 1] = -1;
            }
            ...
            if(doubles_from[r - 1] < 0x10ffff) {
                printf("Range %04X..%04X, LEN=%d ***GAP***\n",
                        doubles_from[r - 1] + 1, 0x10ffff,
                        0x10ffff - doubles_from[r - 1]);
                ++countrange;

                packed_doubleRanges[used_doubleRanges] =
                        MK_SINGGAP(doubles_from[r - 1] + 1, 0x10ffff);
                ++used_doubleRanges;
            }
    ...
    for(k = 0; k < totalcomp; ++k) {
        ...
        for(i = 0; i < countcomb; ++i)
            if(doubles_to2[index_to[k]] == combiners[i])
                break;
        if(i == countcomb) {
        ...
        if(j == k) {
            ++countstarters;
            if(doubles_to1[index_to[k]] < minstarter)
                minstarter = doubles_to1[index_to[k]];
            if(doubles_to1[index_to[k]] > maxstarter)
                maxstarter = doubles_to1[index_to[k]];
    ...
    do {
        r = j + 1;
        while((combiners[r] == combiners[r - 1] + 1) && (r < countcomb))
            ++r;
        ...
        if(r < countcomb) {
            ...
            if(gapend - gapstart + 1 < COMB_THRESHOLD) {
                ...
                printf("Range %04X..%04X, LEN=%d \n", combiners[prevrange],
                        combiners[r - 1],
                        combiners[r - 1] - combiners[prevrange] + 1);
                totalcomb += combiners[r - 1] - combiners[prevrange] + 1;
                ++countrange;

                packed_combiners[used_combiners] =
                        MK_SINGRANGE(combiners[prevrange], combiners[r - 1],
                        used_starteroff);
                ++used_combiners;
                    ...
                    *count = 0;
                    ...
                    for(k = 0; k < totalcomp; ++k) {
                        if(doubles_to2[index_to[k]] == combiners[prevrange + f]) {
                            ...
                            packed_starterData[used_starterdata++] =
                                    doubles_to1[index_to[k]];
                            packed_starterData[used_starterdata++] =
                                    doubles_from[index_to[k]];
                            ++*count;
                        }
                    }
                    // STORE THE OFFSET INTO THE DATA
                    packed_starters[used_starteroff + combiners[f + prevrange] -
                            combiners[prevrange]] = count - packed_starterData;
            ...
            if(combiners[r - 1] < 0x10ffff) {
                printf("Range %04X..%04X, LEN=%d ***GAP***\n",
                        combiners[r - 1] + 1, 0x10ffff,
                        0x10ffff - combiners[r - 1]);
                ++countrange;

                packed_combiners[used_combiners] =
                        MK_SINGGAP(combiners[r - 1] + 1, 0x10ffff);
                ++used_combiners;
            }
