...
int find(uint32_t value)
{
    ...
    {
        if(index0[idx]==value) return idx;
    }
    return -1;
}
...
for(idx=0;idx<COMP_DIGITS_2;++idx)
{
    ...
    if((wordidx==previdx)||(previdx==-1)) { ++wordcount; previdx=wordidx; continue; }
    else {
        // WE HAVE AN INDEX AND A COUNT
        uint8_t byte;
        if(previdx>63) byte=(previdx&0x3f)|0x40;
        else byte=previdx;
        if(wordcount>1) byte|=0x80;
        //else byte|=wordcount<<5;

        compressed_table[offset]=byte;
        ++offset;
        if(previdx>63) { compressed_table[offset]=(previdx>>6)&0xff; ++offset; }
        if(wordcount>1) { compressed_table[offset]=wordcount; ++offset; }
        previdx=wordidx;
        wordcount=1;
    }
    }
    // PROCESS THE LAST WORD
    {
        // WE HAVE AN INDEX AND A COUNT
        uint8_t byte;
        if(wordidx>63) byte=(wordidx&0x3f)|0x40;
        else byte=wordidx;
        if(wordcount>1) byte|=0x80;
        //else byte|=wordcount<<5;
        ...
        compressed_table[offset]=byte;
        ++offset;
        if(wordidx>63) { compressed_table[offset]=(wordidx>>6)&0xff; ++offset; }
        if(wordcount>1) { compressed_table[offset]=wordcount; ++offset; }
    }
...
printf("-------------------------------\n");
printf("Total size of table=%d bytes\n",offset+COMP_DIGITS_2*sizeof(comp_offsets[0])+lastindex*sizeof(index0[0]));
...
for(idx=0;idx<COMP_DIGITS_2;++idx)
{
    ...
    if((wordidx==previdx)||(previdx==-1)) { ++wordcount; previdx=wordidx; continue; }
        ...
        if(frequency[previdx]<3) {
            if(offsetrepeat==-1) {
                ...
                else ++compressed_table[offsetrepeat];
            }
        ...
        offsetrepeat=-1;
        if(previdx>maxidx) maxidx=previdx;

        if(previdx>31) byte=(previdx&0x1f)|0x20;
        else byte=previdx;
        if(wordcount>1) byte|=0x40;
        //else byte|=wordcount<<5;
        compressed_table[offset]=byte;
        ++offset;
        if(previdx>31) { compressed_table[offset]=(previdx>>5)&0xff; ++offset; }
        if(wordcount>1) { compressed_table[offset]=wordcount; ++offset; }
        }
        previdx=wordidx;
        wordcount=1;
    ...
    // PROCESS THE LAST WORD
    {
        ...
        if(wordidx>maxidx) maxidx=wordidx;

        if(wordidx>31) byte=(wordidx&0x1f)|0x20;
        else byte=wordidx;
        if(wordcount>1) byte|=0x40;
        //else byte|=wordcount<<5;
        compressed_table[offset]=byte;
        ++offset;
        if(wordidx>31) { compressed_table[offset]=(wordidx>>5)&0xff; ++offset; }
        if(wordcount>1) { compressed_table[offset]=wordcount; ++offset; }
        }
    }
...
printf("-------------------------------\n");
printf("Total size of table=%d bytes\n",offset+COMP_DIGITS_2*sizeof(comp_offsets[0])+maxidx*sizeof(index0[0]));
...
int findstreamindict(int startidx,int matchlen)
{
    ...
    for(idx=0;idx<lastdict;++idx)
    {
        if(ATAN_TABLE[startidx]==dictionary[idx]) {
            ...
            if(limit>matchlen) limit=matchlen;
            for(count=1;count<limit;++count) {
                if(ATAN_TABLE[startidx+count]!=dictionary[idx+count]) break;
            }
            if(count==matchlen) return idx;
            if(count==limit) {
        ...
        if(ATAN_TABLE[idx]==ATAN_TABLE[startidx]) {
            ...
            for(count=0;count<matchlen;++count)
            {
                if(idx2+count>=endidx)
                    break;
                if(ATAN_TABLE[startidx+count]!=ATAN_TABLE[idx2+count]) break;
            }
...
int getrepeatcount(int matchoffset,int matchlength,int startidx,int endidx)
{
    ...
    while(offset<endidx) {
        count=0;
        while((count<=matchlength) && (ATAN_TABLE[offset+count]==dictionary[matchoffset+count])) ++count;
...
for(idx=0;idx<COMP_DIGITS_2+ONE_EXTRA_VALUE;++idx)
{
    ...
    openmatch=0;
    for(word=0;word<COMP_WORDS;++word) {
    matchlength=findbestmatch(COMP_WORDS_1*idx+1+word,COMP_WORDS_1*(idx+1),&matchrepeat);
    if(matchrepeat*matchlength<1) {
        // ADD WORD TO THE DICTIONARY
        dictionary[lastdict++]=ATAN_TABLE[COMP_WORDS_1*idx+1+word];
        matchlength=1;
        if(openmatch) {
        ++openmatch;
        if(openmatch-1>14) compressed_table[offset-3]=0xf;
        else compressed_table[offset-3]=openmatch-1;
        }
        else {
    ...
    matchoffset=findstreamindict(COMP_WORDS_1*idx+1+word,matchlength);
    ...
    if(matchoffset==-1) {
        ...
        for(count=0;count<matchlength;++count)
            dictionary[lastdict++]=ATAN_TABLE[COMP_WORDS_1*idx+1+word+count];
   }


    // ENCODE IT

    if(matchlength-1>14) byte=15;
    else byte=matchlength-1;
    if(matchrepeat>14) byte|=0xf0;      // matchrepeat ALREADY CONTAINS THE NUMBER OF COPIES NOT INCLUDING THE ORIGINAL STREAM, SO DON'T SUBTRACT ONE
    else byte|=(matchrepeat)<<4;

    compressed_table[offset++]=byte;
    compressed_table[offset++]=matchoffset&0xff;
    compressed_table[offset++]=matchoffset>>8;
    if(matchlength-1>14) compressed_table[offset++]=matchlength-1;
    if(matchrepeat>14) compressed_table[offset++]=matchrepeat;
...
printf("-------------------------------\n");
printf("Total size of table=%d bytes\n",offset+COMP_DIGITS_2*sizeof(comp_offsets[0])+lastdict*sizeof(dictionary[0]));
...
void getreal_table(int exp)
{
    ...
    while(count<COMP_WORDS_1) {
        ...
        repeat=*byte&0xc0;
        ++byte;
        if(repeat&0x40) { _index|=(*byte)<<6; ++byte; }
        if(repeat&0x80) { repeat=*byte; ++byte; }
        else repeat=1;
        while(repeat--) real[count++]=index0[_index];
    }
}
...
void getreal_table2(int exp)
{
    ...
    while(count<COMP_WORDS_1) {
        ...
        repeat=*byte&0x60;
        ++byte;
        if(repeat&0x20) { _index|=(*byte)<<5; ++byte; }
        if(repeat&0x40) { repeat=*byte; ++byte; }
        else repeat=1;
        while(repeat--) real[count++]=index0[_index];
        }
    }
...
void getreal_table3(int exp)
{
    ...
    while(count<COMP_WORDS_1) {
        ...
        if(len>>4==0xf) repeat=*byte++;
        else repeat=len>>4;
        if((len&0xf)==0xf) len=*byte++;
        else len&=0xf;

        while(repeat-->=0) {
            len2=len;
            idx=_index;
            while(len2-->=0) real[count++]=dictionary[idx++];
        }
    }
}
...
int main_compressor()
{
   ...
   for(idx=0;idx<COMP_DIGITS_2+ONE_EXTRA_VALUE;++idx) {
       ...
       for(j=0;j<COMP_WORDS_1;++j) ATAN_TABLE[idx*COMP_WORDS_1+j]=0;

       copy_words((BINT *)ATAN_TABLE+idx*COMP_WORDS_1+1,num.data,num.len);
...
printf("Decompression speed = %.1lf MBytes/sec\n",( (double)COMP_WORDS_1*(double)COMP_DIGITS_2*4.0*1000.0)/((double)end-(double)start)*(double)CLOCKS_PER_SEC/1024.0/1024.0);
...
for(idx=0;idx<lastdict;++idx) {
    ...
    if(idx!=lastdict-1) fprintf(file,", ");
    if(idx%10 == 9) fprintf(file,"\n");
}
fprintf(file,"\n};\n\n");

fprintf(file,"\n\n// TABLE WITH OFFSET OF EACH NUMBER WITHIN THE COMPRESSION STREAM\n\n");

fprintf(file,"uint16_t %s_offsets[%d]= {\n",FILE_NAME,COMP_DIGITS_2+ONE_EXTRA_VALUE);

for(idx=0;idx<COMP_DIGITS_2+ONE_EXTRA_VALUE;++idx) {
    fprintf(file,"%ld",comp_offsets[idx]);
    if(idx!=COMP_DIGITS_2-1+ONE_EXTRA_VALUE) fprintf(file,", ");
    if(idx%10 == 9) fprintf(file,"\n");
}
fprintf(file,"\n};\n\n");
