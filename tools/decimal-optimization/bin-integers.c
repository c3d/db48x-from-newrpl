...
void bIntegerAdd(REAL * res, REAL * a, REAL * b)
{
    ...
    if((a->flags & F_NEGATIVE) != (b->flags & F_NEGATIVE)) {
        ...
        if(a->len > b->len)
            ...
            if(a->len == b->len) {
                int k = a->len - 1;
                while((k > 0) && (a->data[k] == b->data[k]))
                    --k;

                if(((WORD) a->data[k]) >= ((WORD) b->data[k]))
                    largest = a;
                else
                    largest = b;
            }
...
// HORNER'S RULE
    ...
    for(k = 0; k < number->len; ++k) {
        ...
        bbintaddshort(resdata, resdata, number->data[number->len - (k + 1)],
                k + 1);
    ...
    while((resdata[nwords - 1] == 0) && (nwords > 1))
        --nwords;
