...
int generate_atantable(void)
{
    ...
    printf("// Start of atan(2^-k) table generator!\n\n\n");
    ...
    for(k=0;k<CORDIC_TABLESIZE;++k)
    {
        ...
        for(j=0;j<BReg[0].len;++j) {
            ...
            printf("%u%c",atanbin_table[k*MAX_WORDS+j],( (j==CORDIC_TABLEWORDS-1)&&(k==CORDIC_TABLESIZE-1))? ' ':',');
            if(j>0 && ((j%21)==20)) printf("\n");
        }
        for(;j<CORDIC_TABLEWORDS;++j) {
            atanbin_table[k*MAX_WORDS+j]=0;
            printf("0%c",( (j==CORDIC_TABLEWORDS-1)&&(k==CORDIC_TABLESIZE-1))? ' ':',');
            if(j>0 && ((j%21)==20)) printf("\n");
        }
...
int generate_Ktable(void)
{
    ...
    printf("// Start of Product(1/sqrt(1+2^-2k)) table generator!\n\n\n");
    ...
    for(k=0;k<CORDIC_TABLESIZE;++k)
    {
        newRealFromBINT(&RReg[0],5,-1); // 1/2
        newRealFromBINT(&RReg[1],2*k,0); // 2*k

        Context.precdigits=2024;
        powReal(&RReg[2],&RReg[0],&RReg[1]);    // (2^(-2k))

        mulReal(&RReg[3],&RReg[8],&RReg[2]);    // Prod*2^(-2k)
