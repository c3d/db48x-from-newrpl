...
static void binatan_table(int exponent, int sysexp, REAL * real)
{
    ...
    if(exponent >= CORDIC_MAXSYSEXP / 2) {
        ...
        if(exponent & 31)
            real->data[CORDIC_TABLEWORDS - words - 1] =
                    1LL << (32 - (exponent & 31));
        else {
            real->data[CORDIC_TABLEWORDS - words] = 1;
            ++real->len;
...
// Yn=Y+X*tan(Ang)=Y+X*Ang
    ...
    bIntegerMul(ynext, y, z,
            sysexp + ((exponent >> 5) + (startindex >> 5)) * 32);
    bIntegerMul(znext, x, z,
            sysexp + ((exponent >> 5) - (startindex >> 5)) * 32);
...
void bintrig_sincos(REAL * angle, int32_t angmode)
{
    ...
    Context.precdigits =
            (2 * savedprec + 8 >
            REAL_PRECISION_MAX) ? REAL_PRECISION_MAX : (2 * savedprec + 8);
    ...
    if(ltReal(&RReg[0], &RReg[7])) {
        ...
        startexp =
                -RReg[0].exp - ((RReg[0].len - 1) << 3) -
                sig_digits(RReg[0].data[RReg[0].len - 1]) + 1;
    ...
    if(startexp >= savedprec) {
        ...
        sysexp += 31;
        sysexp &= ~31;
        if(sysexp > CORDIC_MAXSYSEXP)
            sysexp = CORDIC_MAXSYSEXP;
        ...
        binCORDIC_Rotational((Context.precdigits >
                    REAL_PRECISION_MAX) ? REAL_PRECISION_MAX +
                8 : Context.precdigits, binstartexp, sysexp);
