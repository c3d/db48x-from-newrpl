 ...
 */
void generate_symbol_table(Elf * elf, Elf_Scn * scn, Elf32_Shdr * shdr)
{
    Elf_Data *data;
    char *name;
    data = 0;
    ...
    for(; esym < lastsym; esym++)
        if(esym->st_value && (ELF32_ST_TYPE(esym->st_info) == STT_FUNC)) {
            ...
            if(strstr(name, ROM_prefix) == name) {
                ROMTable[c].symbol =
                        strdup(strcpy(buf,
                            (char *)((unsigned long)name + prefix_len)));
...
int ROMEntryCmp(const void *a, const void *b)
{
    ROMEntry *_a, *_b;
...
void print_romentries(int sorted)
{
    int i;
    FILE *f;
...
int dump_sections(FILE * out, Elf_file * elf_file)
{
    ...
    for(cnt = 1, elf_file->scn = NULL;
            (elf_file->scn = elf_nextscn(elf_file->elf, elf_file->scn));
            cnt++) {
...
void dump_romlib_version(FILE * out, int version)
{
    char temp[2];
    temp[1] = 0;
    int f;
...
int get_romlib_version(Elf * elf)
{
    Elf_Scn *scn;
    Elf_Data *data;
    Elf32_Shdr *shdr;
...
// FIND CONFIGURATION SYMBOLS WITHIN .config SECTION
    ...
    if(dromlib_addr != -1
            && ((size_t)(dromlib_addr - shdr->sh_addr)) < data->d_size)
        romlib_version =
                *((unsigned int *)((char *)data->d_buf + (dromlib_addr -
                        shdr->sh_addr)));
