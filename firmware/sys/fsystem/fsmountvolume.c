...
//printf("Read2 OK\n");
        ...
        if(TempData2[70] != 0x55 || TempData2[71] != 0xaa) {
            simpfree(TempData); // NOT A BOOT/PARTITION SECTOR!
            return FALSE;
        }

        if((TempData[0] == 0xeb || TempData[0] == 0xe9)
                && (TempData[21] == 0xf8)) {

            if((fs->VolumeAddr == 0) && (VolNumber != 0)) {
                simpfree(TempData);     // FAIL IF NO PARTITIONED CARD
                return FALSE;
            }
...
//printf("Partition analysis\n");
        if(fs->VolumeAddr != 0)
            VolNumber = 0;      // IF THIS IS AN EXTENDED PARTITION, MOUNT ONLY THE FIRST LOGICAL DRIVE
        ...
        {
            simpfree(TempData); // NOT A VALID FAT PARTITION
            return FALSE;
        }
...
// PROCESS BPB
    ...
    if(fs->SectorSize != 512 &&
            fs->SectorSize != 1024 &&
            fs->SectorSize != 2048 && fs->SectorSize != 4096) {
        simpfree(TempData);     // NOT A VALID SECTOR SIZE = BPB CORRUPTED OR NOT THERE
        return FALSE;
    }

    temp = 9;
    fs->SectorSize >>= 9;
    while(fs->SectorSize > 1) {
        fs->SectorSize >>= 1;
        temp++;
    }
    ...
    temp = TempData[13];
    while(temp > 1) {
        ++fs->ClusterSize;      // CONVERT INTO POWER OF 2
        temp >>= 1;
    }
    ...
    if(!fs->TotalSectors)
        fs->TotalSectors = ReadInt32(TempData + 32);
    fs->FATSize = ReadInt16(TempData + 22);
    if(!fs->FATSize)
        fs->FATSize = ReadInt32(TempData + 36);
    ...
    fs->Cluster0Addr =
            fs->VolumeAddr + ((datasect -
                (2 << fs->ClusterSize)) << (fs->SectorSize - 9));
...
// USE ONLY ACTIVE FAT IF MIRRORING IS DISABLED
            fs->NumFATS = 1;
            fs->FirstFATAddr +=
                    ((temp & 0xf) * fs->FATSize) << (fs->SectorSize - 9);
        }

// ROOTDIR FOR FAT32 INIT HERE

        fs->RootDir.FirstCluster = ReadInt32(TempData + 44);
        if(!FSGetChain(fs->RootDir.FirstCluster, &(fs->RootDir.Chain), fs))
            return FALSE;
...
// READ CLUSTER HINT FOR FAT32
        ...
        if(fsinfo < 1 || fsinfo >= fs->TotalSectors)
            fs->FSInfoAddr = 0;
        else {
            fs->FSInfoAddr = fs->VolumeAddr + fsinfo;
// READ THE FSINFO SECTOR
            if(!SDDRead((((uint64_t) fs->FSInfoAddr) << 9), 512, TempData,
                        Disk)) {
                simpfree(TempData);
                return FALSE;
            }
...
// CHECK SIGNATURE
            if((ReadInt32(TempData) == 0x41615252)
                    && (ReadInt32(TempData + 484) == 0x61417272)) {

                fs->FreeSpace = ReadInt32(TempData + 488);
                if(fs->FreeSpace == 0xffffffff)
                    fs->FreeSpace = 0;
                else {
                ...
                if(fs->NextFreeCluster != 0xffffffff) {
                    fs->NextFreeCluster =
                            FSCluster2Addr(fs->NextFreeCluster, fs);
        ...
        fs->RootDir.Chain.StartAddr =
                fs->FirstFATAddr +
                ((fs->NumFATS * fs->FATSize) << (fs->SectorSize - 9));
        fs->RootDir.Chain.EndAddr =
                fs->RootDir.Chain.StartAddr + (rootdir << (fs->SectorSize - 9));
...
int FSUpdateHints(FS_VOLUME * fs)
{
    ...
    // READ THE FSINFO SECTOR
    if(!SDDRead((((uint64_t) fs->FSInfoAddr) << 9), 512, TempData, fs->Disk)) {
        simpfree(TempData);
        return FS_ERROR;
    }
    ...
    if(!SDDWrite((((uint64_t) fs->FSInfoAddr) << 9), 512, TempData, fs->Disk)) {
        simpfree(TempData);
        return FS_ERROR;
    }
