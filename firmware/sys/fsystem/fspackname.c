...
const int const cp850toUnicode[128] = {
    ...
    0XE1, 0XED, 0XF3, 0XFA, 0XF1, 0XD1, 0XAA, 0XBA, 0XBF, 0XAE, 0XAC, 0XBD,
            0XBC, 0XA1, 0XAB, 0XBB,
    0X2591, 0X2592, 0X2593, 0X2502, 0X2524, 0XC1, 0XC2, 0XC0, 0XA9, 0X2563,
            0X2551, 0X2557, 0X255D, 0XA2, 0XA5, 0X2510,
    0X2514, 0X2534, 0X252C, 0X251C, 0X2500, 0X253C, 0XE3, 0XC3, 0X255A, 0X2554,
            0X2569, 0X2566, 0X2560, 0X2550, 0X256C, 0XA4,
    0XF0, 0XD0, 0XCA, 0XCB, 0XC8, 0X131, 0XCD, 0XCE, 0XCF, 0X2518, 0X250C,
            0X2588, 0X2584, 0XA6, 0XCC, 0X2580,
...
void FSPackShortName(char *name, char *direntry)
{
    ...
    if(direntry[12] & 0x8) {
        for(count = 0; count <= f; ++count) {
            ...
            unsigned int cp =
                    cp2utf8(cp850toUnicode[(int)((unsigned char *)
                            direntry)[count] - 128]);
            while(cp) {
                *ptr++ = cp & 0xff;
                cp >>= 8;
