...
//printf("ntype=%02X\n",ntype);
    ...
    if(ntype < 0)
        return FS_BADNAME;
    if(ntype & (FSNAME_DOT | FSNAME_DOTDOT))
        return FS_BADNAME;

    if((ntype & FSNAME_ENDSLASH) || (ntype & FSNAME_EMPTY))
        return FS_BADNAME;
...
// COMPLETE MISSING FIELDS IN ENTRY STRUCTURE
    ...
    file->Attr = attr;
    if(!(cr->NameFlags & 2))
        file->NTRes = cr->NameFlags & (~3);     // SAVE RESERVED CASE INFORMATION
...
// WRITE DIRECTORY INFORMATION
    ...
    k = 1;
    if(cr->NameFlags & 2)
        k += (utf8len((char *)file->Name) + 12) / 13;
...
//printf("short=\"%s\"\n",path);
    ...
    if(!(file->Attr & FSATTR_DIR))
        WriteInt32(path + 28, file->FileSize);
...
// DON'T CARE IF ERROR, ONLY MINOR PROBLEMS TO THE O.S.
            ...
            while(dir->CurrentOffset < dir->FileSize - 511) {
                if(FSWriteLL(nptr, 512, dir, fs) != 512)
                    break;
            }
            simpfree(nptr);
