...
FSPatchFATBlock(buffer,512,bufaddr,fs,FALSE);
	...
	switch(fs->FATType)
	{
		...
		{
		if(addr==-1) fatentry=lostbyte+(buffer[0]<<8);
		else fatentry=ReadInt16((char *)buffer+addr);
		if(firstcluster&1) fatentry>>=4;
		else fatentry&=0xfff;
		if(fatentry!=firstcluster+1) { anotherblock=0; break; }
		if(firstcluster&1) ++addr;
		}
		if(addr==511) lostbyte=buffer[511];		// CARRYOVER FOR NEXT BLOCK
		if(fatentry>0xff7) eoc=1;
		break;
		
		case 2:
		for(;addr<=510;addr+=2,++firstcluster)
		{
		fatentry=ReadInt16((char *)buffer+addr);
		//printf("Entry=%04X fc=%04X\n",fatentry,firstcluster);
		//keyb_getkeyM(1);
		if(fatentry!=firstcluster+1) { 
//		printf("End of fragment\nnewcluster=%d\ncurrent=%d\n",fatentry,firstcluster);
		anotherblock=0; break; }
		}
		if(fatentry>0xfff7) eoc=1;
		break;
		
		case 3:
		for(;addr<=508;addr+=4,++firstcluster)
		{
		fatentry=(*((unsigned int *)(buffer+addr)))&0x0fffffff;
		if(fatentry!=firstcluster+1) { anotherblock=0; break; }
		}
		if(fatentry>0x0ffffff7) eoc=1;
		break;
	}
