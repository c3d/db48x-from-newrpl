...
// CLEAN ENTRY
    ...
    if(mode & (FSMODE_APPEND | FSMODE_MODIFY))
        mode |= FSMODE_WRITE;   // APPEND/MODIFY IMPLY WRITE
...
// CHECK IF CARD IS PRESENT
    ...
    if(error != FS_OK) {
        ...
        if(error == FS_NOTFOUND &&
                (mode & FSMODE_WRITE) && !(mode & FSMODE_NOCREATE)
                ) {
// CREATE FILE
...
// GET FILE CLUSTER CHAIN
    ...
    if((mode & (FSMODE_WRITE | FSMODE_APPEND | FSMODE_MODIFY)) == FSMODE_WRITE)
        entry->FileSize = 0;    // TRUNCATE

    if(mode & FSMODE_APPEND)
        entry->CurrentOffset = entry->FileSize;

    if(entry->Attr & FSATTR_DIR)        // FOR DIRECTORIES, SET FILE SIZE ACCORDING TO CHAIN
    {
        if(entry->FileSize >= 65536 * 32)
            entry->Mode |= FSMODE_NOGROW;
...
// UPDATE FILE ACCESS DATES
    ...
    if(mode & FSMODE_WRITE)
        entry->WriteTimeDate = dtime;   // UPDATE LAST WRITE DATE IF OPENED FOR WRITING
