...
// DIRECTORY FUNCTIONS
extern int FSFindEntry(char *name, int caseflags, FS_FILE * entry,
        FS_FILE * dir);
extern int FSUpdateDirEntry(FS_FILE * file);
extern int FSDeleteDirEntry(FS_FILE * file);
extern void FSMovedOpenFiles(FS_FILE * dir, int entryoffset, int newoffset,
        FS_VOLUME * fs);
extern int FSPackDir(FS_FILE * dir);
extern int FSFindForCreation(char *name, FS_FILECREATE * cr, FS_FILE * dir);

// FILE FUNCTIONS
extern int FSReadLL(unsigned char *buffer, int nbytes, FS_FILE * file,
        FS_VOLUME * fs);
extern int FSWriteLL(unsigned char *buffer, int nbytes, FS_FILE * file,
        FS_VOLUME * fs);
...
extern void FSPatchFATBlock(unsigned char *buffer, int size, int addr,
        FS_VOLUME * fs, int flush);
