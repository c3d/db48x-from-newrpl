...
int FSWrite(unsigned char *buffer, int nbytes, FS_FILE * file)
{
    ...
    if(!(file->Mode & FSMODE_WRITE))
        return FS_ERROR;        // WRITE NOT ALLOWED
    if(file->Attr & FSATTR_RDONLY)
        return FS_ERROR;

    if(file->Mode & FSMODE_APPEND)
        file->CurrentOffset = file->FileSize + file->WrBuffer.Used;     // ONLY AT THE END OF FILE
    ...
    while((file->Mode & FSMODE_WRITEBUFFERS) && !(file->Mode & FSMODE_MODIFY)) {
        ...
        if(file->WrBuffer.Used) {
            if((file->CurrentOffset < file->WrBuffer.Offset)
                    || (file->CurrentOffset > file->WrBuffer.Offset + 512)
                    || (file->CurrentOffset >
                        file->WrBuffer.Offset + file->WrBuffer.Used)) {
                ...
                int written =
                        FSWriteLL(file->WrBuffer.Data, file->WrBuffer.Used,
                        file, FSystem.Volumes[file->Volume]);
                if(!(file->Mode & FSMODE_MODIFY)) {
                    file->FileSize = file->CurrentOffset;       // TRUNCATE FILE
                }
                if(written != file->WrBuffer.Used)
                    return FS_ERROR;
                file->CurrentOffset = oldoffset;
                file->WrBuffer.Used = 0;
            }
            else {
                if(file->CurrentOffset <
                        file->WrBuffer.Offset + file->WrBuffer.Used) {
                ...
                if(nbytes > maxbytes) {
                    memmoveb(file->WrBuffer.Data + file->WrBuffer.Used, buffer,
                            maxbytes);
        ...
        if(nbytes > remainder) {
            int written =
                    FSWriteLL(buffer, nbytes - remainder, file,
                    FSystem.Volumes[file->Volume]);
            if(!(file->Mode & FSMODE_MODIFY)) {
                file->FileSize = file->CurrentOffset;   // TRUNCATE FILE
            }
            totalwritten += written;
            if(written != nbytes - remainder)
                return totalwritten;
