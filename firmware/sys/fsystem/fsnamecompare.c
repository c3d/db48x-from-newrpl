...
// CASE-SENSITIVE W/SEMICOLON STRIPPING
        ...
        n1--;
        n2--;
        while((n1 > (unsigned char *)name1) && (*n1 == ';'))
            --n1;
        while((n2 > (unsigned char *)name2) && (*n2 == ';'))
            --n2;
        if((int)(n2 - (unsigned char *)name2) !=
...
// COMPARE EXTENSIONS FIRST
    ...
    if(*ext1 != *ext2)
        return -1;      // RETURN IF EXTENSIONS HAVE DIFFERENT LENGTH
...
// SAME EXTENSION, NOW CHECK FOR NAME
    ...
    while(*shentry && *newname) {
        ...
        if(*newname == '~') {
            ...
            while(*shentry != 0 && *shentry != '.' && *shentry != '~')
                ++shentry;
            if(*shentry != '~')
                return 0;       // NO NUMBER ON shentry
    ...
    if(*shentry == 0 && *newname == 0)
        return 0;       // IDENTICAL ROOTS, NO NUMBER
