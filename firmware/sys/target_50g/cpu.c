...
int __cpu_getFCLK()
{
    ...
    else        // fast mode
    {
        ...
        int m = ((PLLCON >> 12) & 0xff) + 8, p = ((PLLCON >> 4) & 0x1f) + 2, s =
                PLLCON & 3;
...
// TURN LCD OFF
    ...
    if(PLLCON > 0xffffff) {
        ...
        // SWITCH TO SLOW MODE
        *HWREG(CLK_REGS, 0x10) =
                (*HWREG(CLK_REGS, 0x10) & (~0x37)) | (PLLCON >> 24);
    ...
    *HWREG(CLK_REGS, 0x4) = PLLCON;
    ...
    if(FCLK <= 24000000)
        *HWREG(CLK_REGS, 0x14) = 0;     // SET HCLK=FCLK; PCLK=HCLK;
    else if(FCLK <= 48000000)
        *HWREG(CLK_REGS, 0x14) = 1;     // SET HCLK=FCLK; PCLK=HCLK/2;
    else if(FCLK <= 75000000)
        *HWREG(CLK_REGS, 0x14) = 2;     // SET HCLK=FCLK/2; PCLK=HCLK;
    else
