...
void battery_handler()
{
    ...
    if (RECORDER_TWEAK(battery_debug))
    {
        ...
        DrawTextBk(STATUSAREA_X,SCREEN_HEIGHT-2*H,text,font,ggl_mkcolor(PAL_STATEXT),ggl_mkcolor(PAL_STABACKGND),&scr);
        ...
        DrawTextBk(STATUSAREA_X,SCREEN_HEIGHT-H,text,font,ggl_mkcolor(PAL_STATEXT),ggl_mkcolor(PAL_STABACKGND),&scr);
    }
    ...
    if(!bat_readcnt) {
        ...
        if(battery==0x400) {
            ...
            DrawTextBk(SCREEN_WIDTH-StringWidth((char *)"C", Font_Notifications)-1, SCREEN_HEIGHT-1-FONT_Notifications.BitmapHeight, (char *)"C",
                       Font_Notifications, ggl_mkcolor(PAL_STABAT), ggl_mkcolor(PAL_STABACKGND), &scr);
        }
        else {
            ...
            DrawTextBk(SCREEN_WIDTH-percentwidth,SCREEN_HEIGHT-FONT_10A.BitmapHeight-1,(char *)&text,Font_10A,ggl_mkcolor(PAL_STABAT), ggl_mkcolor(PAL_STABACKGND),&scr);
