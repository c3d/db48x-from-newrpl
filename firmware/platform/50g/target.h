...
#define HWREG(base, off)         ((volatile uint32_t *) (((uintptr_t) base + (uintptr_t) off)))
...
#define GPFDAT                   HWREG(IO_REGS, 0x54)
...
#define STR_PRODUCT                                                          \
    {                                                                        \
        'n', 0, 'e', 0, 'w', 0, 'R', 0, 'P', 0, 'L', 0, ' ', 0, '5', 0, '0', \
            0, 'g', 0                                                        \
    }
...
#define MEM_PHYSTACK \
    0x40000ffc // PHYSICAL LOCATION OF THE "C" STACK (TOP OF STACK, DECREASES
               // DOWN TO 0x40000000)
#define MEM_ROM \
    0x00000000 // VIRTUAL (AND PHYSICAL) ROM LOCATION (UP TO 4 MBytes)
...
#define MEM_TEMPBLOCKS \
    0x01400000 // BLOCK INDEX FOR TEMPOB VIRTUAL LOCATION (UP TO 4 MB)
#define MEM_TEMPOB \
    0x01800000 // GLOBAL OBJECT ALLOCATION MEMORY VIRTUAL LOCATION (UP TO 8 MB)
#define MEM_SYSTEM \
    0x02000000 // SYSTEM RAM (FIXED AMOUNT, MAPPED AT THE BEGINNING OF THE
               // PHYSICAL RAM)
...
/* PHYSICAL RAM LAYOUT:
  ...
  // FIRST 32 KBYTES ARE USED BY THE BOOTLOADER, SO NOTHING PERSISTENT CAN GO
 THERE.
  ...
  0x08000100 - SCRATCH MEMORY (RREG MEMORY = 504 WORDS PER REGISTER, 8
 REGISTERS) 0x08005000 - SYSTEM GLOBAL VARIABLES (4 KB) 0x08006000 - 0x08007900h
 SCREEN (8 KB)

  // PERSISTENT MEMORY
  0x08008000 - MMU TABLE BASE (1 KB) (MUST BE 32 KBYTE ALIGNED) - 1K MAPS FROM
 0x000|00000 TO 0x0FF|00000 INCLUSIVE 0x08008400 - DSTK 2ND TABLE (1 KB) 1K MAPS
 256 4K PAGES = 1 MB OF RAM 0x08008800 - RSTK 2ND TABLE (1 KB) 0x08008C00 - LAM
 2ND TABLE (1 KB) 0x08009000 - DIRS 2ND TABLE (1 KB) 0x08009400 - TEMPBLOCKS 2ND
 TABLE (1 KB) 0x08009800 - TEMPOB 2ND TABLE (4 KB) 1K MAPS 4 MB OF RAM MAX
 (SPACE LEFT FOR FUTURE VIRTUAL MEMORY) 0x0800A800 - REVERSE WALK TABLE (1 KB) -
 REVERSE MAP PHYSICAL TO VIRTUAL - INDICATES WHICH RAM PAGES ARE USED/FREE
  0x0800AC00 - 1 KB = PERSISTENT VARIABLES OF THE RPL CORE + MPD library MALLOC
 AREA (32K) 0x0800XXXX - FIRST AVAILABLE PAGE OF RAM, AFTER END OF ALL
 PERSISTENT VARIABLES, ROUNDED TO 4K

 TOTAL USED FOR SYSTEM + MMU TABLES = 26 KBYTES (484 KBYTES AVAILABLE FOR THE
 USER)
