...
// SEND LOW SIGNAL ON ALL SD LINES
        ...
        *HWREG(CLK_REGS, 0xc) = *HWREG(CLK_REGS, 0xc) & (~0x200);       // DISABLE CLOCK TO SD INTERFACE
...
int SDSetFastPCLK()
{
    unsigned original = *HWREG(CLK_REGS, 0x14) & 1;

    *HWREG(CLK_REGS, 0x14) &= ~1;
...
{
    *HWREG(CLK_REGS, 0x14) = (*HWREG(CLK_REGS, 0x14) & (~1)) | (original & 1);
}
...
int SDDSetBlockLen(SD_CARD * card, int bitlen)
{
    ...
    if(bitlen > card->MaxBlockLen)
        bitlen = card->MaxBlockLen;

    if(bitlen == card->CurrentBLen)
        return TRUE;

    if(!SDSendCmdShortResp(16, 1 << bitlen, &a))
        return FALSE;
...
int
SDDRead(uint64_t SDAddr, int NumBytes, unsigned char *buffer, SD_CARD * card)
{
  ...
  restartall:
    ...
    blocks = ((endaddr + (1 << card->CurrentBLen) -
...
// EXIT BYTE LOOP
        ...
        do
        {
            ...
            while((r1 = *SDIFSTA) != r2)
                r2 = *SDIFSTA;  // READ UNTIL STABLE VALUE
            while((r3 = *SDIDSTA) != r4)
                r4 = *SDIDSTA;  // READ UNTIL STABLE VALUE
...
//printf("FSTA=%04X\n",*SDIFSTA);
            if(r1 & 0x1000)
                *((unsigned char *)SDIDCNT) = *((unsigned char *)SDIDAT);
...
// GET CSD TO OBTAIN TOTAL SIZE OF CARD, SECTOR AND BLOCK LENGTH
    if(!SDSendCmdLongResp(9, card->Rca, (int *)CSD)) {
...
// DISCONNECT CARD DETECT PULL-UP RESISTOR ON DAT3
    SDSendACmdShortResp(card->Rca, 42, 0, (int *)CSD);

// REQUEST SCR REGISTER FOR BUS WIDTH
    if(!SDSendACmdLongResp(card->Rca, 51, card->Rca, (int *)CSD)) {
...
// ENABLE WIDE BUS SUPPORT
        if(!SDSendACmdShortResp(card->Rca, 6, 2, (int *)CSD))
        {
            halFlags &= ~HAL_NOCLOCKCHANGE;
...
// CARD MUST BE SELECTED
int
SDDWrite(uint64_t SDAddr, int NumBytes, unsigned char *buffer, SD_CARD * card)
{
    ...
    if(startaddr != SDAddr) {
        ...
        if(SDDRead(startaddr, blmask + 1, startbuffer, card) != blmask + 1) {

            simpfree(startbuffer);
        ...
        if(blocks > 1 || (!startbuffer))
        {
            ...
            if(SDDRead
                    (finalblock
                        -
                        (blmask
                            + 1), 1 << card->CurrentBLen, endbuffer,
                        card) != blmask + 1) {
...
//printf("blocks=%d\n",blocks);
    ...
    if(!SDSendCmdShortResp
            ((blocks ==
                    1) ?
                24 : 25,
                (int)((card->SysFlags & 16) ? (startaddr >> 9) : startaddr),
                &status))
    {
//printf("failed read cmd\n");
        if(startbuffer)
        ...
        if(endbuffer && endbuffer != startbuffer)
            simpfree(endbuffer);
...
//printf("First loop done\n");
    ...
    if(((unsigned int)SDAddr | (unsigned int)endaddr | (unsigned int)buffer) &
            3) {
// SLOW BYTE-ALIGNED LOOP
...
//printf("Slowdown failed\n");
            ...
            else
            {
                ...
                {
                    *SDIDAT =
                            *(unsigned int *)(endbuffer + (startaddr & blmask));
                }
                else
                {

                    *SDIDAT = *((unsigned int *)buffer);
