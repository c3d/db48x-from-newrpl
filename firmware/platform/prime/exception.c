...
{
    return StringWidth(string, Font_10A);
}
...
doitagain:
    ...
    else
    {
        ...
        else
        {
            ...
            {
                ex_print(LCD_W / 2 + 19 * FNT_AVGW, 2 * FNT_HEIGHT, "N");
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 2 * FNT_HEIGHT, "MI");
            }
            else
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 2 * FNT_HEIGHT, "PL");
            if (hi_reg[7] & 0x40000000)
            {
                ex_print(LCD_W / 2 + 19 * FNT_AVGW, 3 * FNT_HEIGHT, "Z");
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 3 * FNT_HEIGHT, "EQ");
            }
            else
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 3 * FNT_HEIGHT, "NE");
            if (hi_reg[7] & 0x20000000)
            {
                ex_print(LCD_W / 2 + 19 * FNT_AVGW, 4 * FNT_HEIGHT, "C");
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 4 * FNT_HEIGHT, "CS");
            }
            else
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 4 * FNT_HEIGHT, "CC");
            if (hi_reg[7] & 0x10000000)
            {
                ex_print(LCD_W / 2 + 19 * FNT_AVGW, 5 * FNT_HEIGHT, "V");
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 5 * FNT_HEIGHT, "VS");
            }
            else
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 5 * FNT_HEIGHT, "VC");
            if ((hi_reg[7] & 0x60000000) == 0x20000000)
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 6 * FNT_HEIGHT, "HI");
            else
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 6 * FNT_HEIGHT, "LS");
            if ((hi_reg[7] ^ (hi_reg[7] >> 3)) & 0x10000000)
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 7 * FNT_HEIGHT, "LT");
            else if (!(hi_reg[7] & 0x40000000))
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 7 * FNT_HEIGHT, "GT");
            ...
            if (hi_reg[7] & 0x20)
                ex_print(LCD_W / 2 + 15 * FNT_AVGW, 10 * FNT_HEIGHT, "Thumb");
            else
                ex_print(LCD_W / 2 + 16 * FNT_AVGW, 10 * FNT_HEIGHT, "ARM");
        }
    ...
    if (options & EX_WARM)
    {
        ...
        if (options & EX_WIPEOUT)
            ex_print(1 * BTN_WIDTH + 4, (LCD_H - FNT_HEIGHT), "*Clear Mem*");
        else
            ex_print(1 * BTN_WIDTH + 4, (LCD_H - FNT_HEIGHT), "*Warmstart*");
...
ARM_MODE void handler_und(void)
{
    ...
    else
    {
        // CALL CUSTOM HANDLER
        if (value == 0xe6dddd10)
            value = exception_handler((char *) (stackptr[1]), stackptr, stackptr[2]);
        else
