...
word_p *halGrowMemory(int32_t zone, word_p * base, int32_t newsize)
{
    ...
    if(newsize > current) {
        ...
        int *mmutable = (int *)MEM_REVERSEMMU;
        while(needed && (mmutable < (int *)(MEM_REVERSEMMU + 0x8000))) {
            if(*mmutable == 0) {
...
// AND ALL OTHERS AS FREE (1 kbytes RESERVED FOR THE TABLE, ONLY 512 BYTES USED FOR NOW)
    while(mmutable < (int *)(MEM_REVERSEMMU + 0x8000))
        *mmutable++ = 0;

// SINCE ALL PAGES ARE FREE, CLEAR ALL MMU TABLES

    memsetw((void *)MEM_RSTKMMU, 0, 4096);
    memsetw((void *)MEM_DSTKMMU, 0, 4096);
    memsetw((void *)MEM_LAMMMU, 0, 4096);
    memsetw((void *)MEM_DIRMMU, 0, 4096);
    memsetw((void *)MEM_TEMPBLKMMU, 0, 4096);
    memsetw((void *)MEM_TEMPOBMMU, 0, 4096);
...
int halCheckMemoryMap()
{
    ...
    int *mmutable = (int *)MEM_REVERSEMMU;
    int virt = RAM_BASE_PHYSICAL;

    // CHECK INITIAL USED PAGES TO VERIFY IF VALID MMU MAP EXISTS
    while(virt < end) {
        if(mmutable[(virt - RAM_BASE_PHYSICAL) >> 12] != virt) {
    ...
    while((*mmu != 0) && ((mmu - (int *)MEM_RSTKMMU) < 0x1000)) {
        page = *mmu;
    ...
    while((*mmu != 0) && ((mmu - (int *)MEM_DSTKMMU) < 0x1000)) {
        page = *mmu;
        if((page < RAM_BASE_PHYSICAL) || (page > RAM_END_PHYSICAL)) {
    ...
    while((*mmu != 0) && ((mmu - (int *)MEM_DIRMMU) < 0x1000)) {
        page = *mmu;
        if((page < RAM_BASE_PHYSICAL) || (page > RAM_END_PHYSICAL)) {
    ...
    while((*mmu != 0) && ((mmu - (int *)MEM_LAMMMU) < 0x1000)) {
        page = *mmu;
        if((page < RAM_BASE_PHYSICAL) || (page > RAM_END_PHYSICAL)) {
    ...
    while((*mmu != 0) && ((mmu - (int *)MEM_TEMPBLKMMU) < 0x1000)) {
        page = *mmu;
        if((page < RAM_BASE_PHYSICAL) || (page > RAM_END_PHYSICAL)) {
    ...
    while((*mmu != 0) && ((mmu - (int *)MEM_TEMPOBMMU) < 0x1000)) {
        page = *mmu;
        if((page < RAM_BASE_PHYSICAL) || (page > RAM_END_PHYSICAL)) {
