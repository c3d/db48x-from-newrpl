...
void ggl_cliprect(gglsurface *srf,int x1,int y1,int x2,int y2,int color)
{
    ...
    if(x1>srf->clipx2) return;
    if(y1>srf->clipy2) return;
    if(y2<srf->clipy) return;
    if(x2<srf->clipx) return;

    if(x1<srf->clipx) x1=srf->clipx;
    if(y1<srf->clipy) y1=srf->clipy;
    if(x2>srf->clipx2) x2=srf->clipx2;
    if(y2>srf->clipy2) y2=srf->clipy2;
